Meteor.methods
	...
	push_test: ->
		...
		if not TAGT.authz.hasRole(user._id, 'admin')
			throw new Meteor.Error 'error-not-allowed', 'Not allowed', { method: 'push_test' }
...
configurePush = ->
	if TAGT.settings.get 'Push_debug'
		Push.debug = true
		console.log 'Push: configuring...'

	if TAGT.settings.get('Push_enable') is true
		Push.allow
			send: (userId, notification) ->
				return TAGT.authz.hasRole(userId, 'admin')
		...
		if TAGT.settings.get('Push_enable_gateway') is false
			gcm =
				...
				projectNumber: TAGT.settings.get 'Push_gcm_project_number'

			apn =
				passphrase: TAGT.settings.get 'Push_apn_passphrase'
				keyData: TAGT.settings.get 'Push_apn_key'
				certData: TAGT.settings.get 'Push_apn_cert'

			if TAGT.settings.get('Push_production') isnt true
				apn =
					passphrase: TAGT.settings.get 'Push_apn_dev_passphrase'
					keyData: TAGT.settings.get 'Push_apn_dev_key'
					certData: TAGT.settings.get 'Push_apn_dev_cert'
		...
		Push.Configure
			apn: apn
			gcm: gcm
			production: TAGT.settings.get 'Push_production'
			sendInterval: 1000
			sendBatchSize: 10

		if TAGT.settings.get('Push_enable_gateway') is true
...
sendPush = (service, token, options, tries=0) ->
	...
	HTTP.post TAGT.settings.get('Push_gateway') + "/push/#{service}/send", data, (error, response) ->
