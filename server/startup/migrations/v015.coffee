TAGT.Migrations.add
	...
	up: ->
		...
		newFilesCollection  = TAGT.models.Uploads
		newGridFSCollection = new Meteor.Collection 'tagt_uploads.files'
		newChunkCollection  = new Meteor.Collection 'tagt_uploads.chunks'


		oldFilesCollection.find({'copies.files.key': {$exists: true}}).forEach (cfsRecord) ->
			nameParts = cfsRecord.original.name?.split('.')
			extension = ''
			url = "ufs/tagt_uploads/#{cfsRecord._id}"
			...
			TAGT.models.Messages.find({$or: [{ 'urls.url': "https://demo.talk.get/cfs/files/Files/#{cfsRecord._id}" }, { 'urls.url': "https://caoliao.net.cn/cfs/files/Files/#{cfsRecord._id}" }]}).forEach (message) ->
				for urlsItem in message.urls
					if urlsItem.url is "https://demo.talk.get/cfs/files/Files/#{cfsRecord._id}" or urlsItem.url is "https://caoliao.net.cn/cfs/files/Files/#{cfsRecord._id}"
						urlsItem.url = Meteor.absoluteUrl() + url
						if urlsItem.parsedUrl?.pathname?
							urlsItem.parsedUrl.pathname = "/#{url}"
						message.msg = message.msg.replace "https://demo.talk.get/cfs/files/Files/#{cfsRecord._id}", Meteor.absoluteUrl() + url
						message.msg = message.msg.replace "https://caoliao.net.cn/cfs/files/Files/#{cfsRecord._id}", Meteor.absoluteUrl() + url

				TAGT.models.Messages.update {_id: message._id}, {$set: {urls: message.urls, msg: message.msg}}
