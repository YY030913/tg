TAGT.Migrations.add({
	version: 40,
	up: function() {
		TAGT.models.Settings.find({ _id: /Accounts_OAuth_Custom_/, i18nLabel: 'Accounts_OAuth_Custom_Enable' }).forEach(function(customOauth) {
			var parts = customOauth._id.split('_');
			var name = parts[3];
			var id = 'Accounts_OAuth_Custom_' + name + '_token_sent_via';
			if (!TAGT.models.Settings.findOne({ _id: id })) {
				TAGT.models.Settings.insert({
					'_id': id,
					'type': 'select',
					'group': 'OAuth',
