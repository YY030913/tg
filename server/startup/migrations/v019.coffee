TAGT.Migrations.add
	...
	up: ->
		...
		TAGT.authz.addUsersToRoles( _.pluck(admins, '_id'), ['admin'])
		...
		TAGT.authz.addUsersToRoles( _.pluck(users, '_id'), ['user'])
		...
		rooms = TAGT.models.Rooms.findByTypes(['c','p']).fetch()
		_.each rooms, (room) ->
			creator = room?.u?._id
			if creator
				if Meteor.users.findOne({_id: creator})
					TAGT.authz.addUsersToRoles( creator, ['moderator'], room._id)
				else
