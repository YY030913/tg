Meteor.startup ->
	...
	if TAGT.settings.get 'Accounts_AvatarStoreType'
		storeType = TAGT.settings.get 'Accounts_AvatarStoreType'

	TAGTStore = TAGTFile[storeType]
	...
	transformWrite = (file, readStream, writeStream) ->
		if TAGTFile.enabled is false or TAGT.settings.get('Accounts_AvatarResize') isnt true
			return readStream.pipe writeStream

		height = TAGT.settings.get 'Accounts_AvatarSize'
		width = height

		TAGTFile.gm(readStream, file.fileName).background('#ffffff').resize(width, height+'^>').gravity('Center').extent(width, height).stream('jpeg').pipe(writeStream)

	path = "~/uploads"

	if TAGT.settings.get('Accounts_AvatarStorePath')?.trim() isnt ''
		path = TAGT.settings.get 'Accounts_AvatarStorePath'

	@TAGTFileAvatarInstance = new TAGTStore
		name: 'avatars'
		absolutePath: path
	...
	WebApp.connectHandlers.use '/avatar/', Meteor.bindEnvironment (req, res, next) ->
		...
		if params.username[0] isnt '@'
			if Meteor.settings?.public?.sandstorm
				user = TAGT.models.Users.findOneByUsername(params.username.replace('.jpg', ''))
				if user?.services?.sandstorm?.picture
					res.setHeader 'Location', user.services.sandstorm.picture
					res.writeHead 302
					res.end()
					return
			file = TAGTFileAvatarInstance.getFileWithReadStream encodeURIComponent(params.username)
		else
