@getAvatarSuggestionForUser = (user) ->
	...
	if user.services.facebook?.id? and TAGT.settings.get 'Accounts_OAuth_Facebook'
		avatars.push
			service: 'facebook'
			url: "https://graph.facebook.com/#{user.services.facebook.id}/picture?type=large"

	if user.services.google?.picture? and user.services.google.picture isnt "https://lh3.googleusercontent.com/-XdUIqdMkCWA/AAAAAAAAAAI/AAAAAAAAAAA/4252rscbv5M/photo.jpg" and TAGT.settings.get 'Accounts_OAuth_Google'
		avatars.push
			service: 'google'
	...
	if user.services.github?.username? and TAGT.settings.get 'Accounts_OAuth_Github'
		avatars.push
			service: 'github'
			url: "https://avatars.githubusercontent.com/#{user.services.github.username}?s=200"

	if user.services.linkedin?.pictureUrl? and TAGT.settings.get 'Accounts_OAuth_Linkedin'
		avatars.push
			service: 'linkedin'
			url: user.services.linkedin.pictureUrl

	if user.services.twitter?.profile_image_url_https? and TAGT.settings.get 'Accounts_OAuth_Twitter'
		avatars.push
			service: 'twitter'
			url: user.services.twitter.profile_image_url_https

	if user.services.gitlab?.avatar_url? and TAGT.settings.get 'Accounts_OAuth_Gitlab'
		avatars.push
			service: 'gitlab'
