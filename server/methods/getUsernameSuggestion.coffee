...
usernameIsAvaliable = (username) ->
	...
	return not TAGT.models.Users.findOneByUsername({$regex : new RegExp("^" + username + "$", "i") })
...
@generateSuggestion = (user) ->
	...
	if Match.test(user?.name, String)
		if TAGT.settings.get 'UTF8_Names_Slugify'
	...
	return undefined
TAGT.generateUsernameSuggestion = generateSuggestion
