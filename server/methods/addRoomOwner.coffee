Meteor.methods
	addRoomOwner: (rid, userId) ->
		...
		TAGT.models.Subscriptions.addRoleById(subscription._id, 'owner')
		...
		if TAGT.settings.get('UI_DisplayRoles')
			TAGT.Notifications.notifyAll('roles-change', { type: 'added', _id: 'owner', u: { _id: user._id, username: user.username }, scope: rid });
