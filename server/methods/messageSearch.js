Meteor.methods({
	messageSearch: function(text, rid, limit) {
		...
		if (text !== '') {
			...
			} else if (TAGT.settings.get('Message_AlwaysSearchRegExp')) {
				query.msg = {
					$regex: text,
					$options: 'i'
				};
		...
		if (Object.keys(query).length > 0) {
			...
			if (rid != null) {
				...
				if (Meteor.call('canAccessRoom', rid, currentUserId) !== false) {
					if (!TAGT.settings.get('Message_ShowEditedStatus')) {
						options.fields = {
							'editedAt': 0
						};
					}
					result.messages = TAGT.models.Messages.find(query, options).fetch();
				}
			}
		}
