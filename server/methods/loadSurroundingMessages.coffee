Meteor.methods
	loadSurroundingMessages: (message, limit=50) ->
		...
		message = TAGT.models.Messages.findOneById(message._id);
		...
		records = TAGT.models.Messages.findVisibleByRoomIdBeforeTimestamp(message.rid, message.ts, options).fetch()
		...
		records = TAGT.models.Messages.findVisibleByRoomIdAfterTimestamp(message.rid, message.ts, options).fetch()
