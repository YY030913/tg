Meteor.methods({
	groupsList: function(nameFilter, limit, sort) {
		...
		if (nameFilter) {
			return { groups: TAGT.models.Rooms.findByTypeAndNameContainingUsername('p', new RegExp(s.trim(s.escapeRegExp(nameFilter)), 'i'), Meteor.user().username, options).fetch() };
		} else {
			let roomIds = _.pluck(TAGT.models.Subscriptions.findByTypeAndUserId('p', Meteor.userId()).fetch(), 'rid');
			return { groups: TAGT.models.Rooms.findByIds(roomIds, options).fetch() };
		}
	}
});
