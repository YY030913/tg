...
Api.addRoute 'version', authRequired: false,
	get: ->
		version = {api: '0.1', tagt: '0.5'}
		status: 'success', versions: version

Api.addRoute 'publicRooms', authRequired: true,
	get: ->
		rooms = TAGT.models.Rooms.findByType('c', { sort: { msgs:-1 } }).fetch()
...
Api.addRoute 'joinedRooms', authRequired: true,
	get: ->
		rooms = TAGT.models.Rooms.findByContainigUsername(@user.username).fetch()
		status: 'success', rooms: rooms
	...
	get: ->
		try
			...
			if Meteor.call('canAccessRoom', rid, this.userId)
				msgs = TAGT.models.Messages.findVisibleByRoomId(rid,
					sort:
						ts: -1
					skip: skip
					limit: limit
...
Api.addRoute 'rooms/:id/online', authRequired: true,
	get: ->
		room = TAGT.models.Rooms.findOneById @urlParams.id
		online = TAGT.models.Users.findUsersNotOffline(fields:
			username: 1
			status: 1).fetch()
		onlineInRoom = []
	...
	for user, i in users
		if user.name?
			if user.email?
				if user.pass?
					try
						nameValidation = new RegExp '^' + TAGT.settings.get('UTF8_Names_Validation') + '$', 'i'
					catch
...
Api.addRoute 'bulk/register', authRequired: true,
	post:
		...
		action: ->
			if TAGT.authz.hasPermission(@userId, 'bulk-register-user')
				try
...
Api.addRoute 'bulk/createRoom', authRequired: true,
	post:
		...
		action: ->
			...
			if TAGT.authz.hasPermission(@userId, 'bulk-create-c')
				try
