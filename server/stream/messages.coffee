...
msgStream.allowRead (eventName) ->
	try
		...
		if room.t is 'c' and not TAGT.authz.hasPermission(this.userId, 'preview-c-room') and room.usernames.indexOf(room.username) is -1
			return false
...
Meteor.startup ->
	...
	TAGT.models.Messages.on 'change', (type, args...) ->
		records = TAGT.models.Messages.getChangedRecords type, args[0], fields
