Template.createCombinedFlex.helpers
	...
	privateSwitchDisabled: ->
		return if TAGT.authz.hasAllPermission ['create-c', 'create-p'] then '' else 'disabled'
	privateSwitchChecked: ->
		return if TAGT.authz.hasAllPermission 'create-c' then '' else 'checked'
	...
	'click .save-channel': (e, instance) ->
		...
		if not err
			Meteor.call createRoute, name, instance.selectedUsers.get(), readOnly, (err, result) ->
				...
				if not privateGroup
					TAGT.callbacks.run 'aftercreateCombined', { _id: result.rid, name: name }
