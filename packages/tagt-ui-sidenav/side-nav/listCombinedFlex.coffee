Template.listCombinedFlex.helpers
	...
	member: ->
		return !!TAGT.models.Subscriptions.findOne({ name: @name, open: true })
	hidden: ->
		return !!TAGT.models.Subscriptions.findOne({ name: @name, open: false })
	roomIcon: ->
		return TAGT.roomTypes.getIcon @t
	url: ->
		return if @t is 'p' then 'group' else 'channel'
	canCreate: ->
		return TAGT.authz.hasAtLeastOnePermission ['create-c', 'create-p']
...
Template.listCombinedFlex.events
	...
	'click footer .create': ->
		if TAGT.authz.hasAtLeastOnePermission( 'create-c')
		...
		if @show.get() is 'joined'
			...
			if _.trim(@channelType.get())
				switch @channelType.get()
					...
					when 'private'
						type = 'p'
			@channelsList.set TAGT.models.Subscriptions.find({
