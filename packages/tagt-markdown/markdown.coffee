...
class Markdown
	constructor: (message) ->
		...
		schemes = TAGT.settings.get('Markdown_SupportSchemesForLink').split(',').join('|')
		...
		if TAGT.settings.get('Markdown_Headers')
			# Support # Text for h1
...
TAGT.Markdown = Markdown
TAGT.callbacks.add 'renderMessage', Markdown    , TAGT.callbacks.priority.HIGH

if Meteor.isClient
	Blaze.registerHelper 'TAGTMarkdown', (text) ->
		return TAGT.Markdown text
