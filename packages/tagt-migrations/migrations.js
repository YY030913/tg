...
Migrations = {
	...
	options: {
		...
		collectionName: "migrations"
			// collectionName: "tagt_migrations"
	},
...
Migrations.migrateTo = function(command) {
	...
	if (!migrated) {
		...
		console.log(makeABox([
			...
			"",
			"This TalkGet version: " + TAGT.Info.version,
			"Database locked at version: " + control.version,
			"Database target version: " + (version === 'latest' ? _.last(this._list).version : version),
			"",
			"Commit: " + TAGT.Info.commit.hash,
			"Date: " + TAGT.Info.commit.date,
			"Branch: " + TAGT.Info.commit.branch,
			"Tag: " + TAGT.Info.commit.tag
		]));
		process.exit(1);
	}
...
Migrations._migrateTo = function(version, rerun) {
	...
	function lock() {
		...
		const build = TAGT.Info ? TAGT.Info.build.date : date;
