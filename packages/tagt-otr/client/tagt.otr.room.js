...
TAGT.OTR.Room = class {
	...
	handshake(refresh) {
		...
		this.generateKeyPair().then(() => {
			TAGT.Notifications.notifyUser(this.peerId, 'otr', 'handshake', { roomId: this.roomId, userId: this.userId, publicKey: EJSON.stringify(this.exportedPublicKey), refresh: refresh });
		});
	}

	acknowledge() {
		TAGT.Notifications.notifyUser(this.peerId, 'otr', 'acknowledge', { roomId: this.roomId, userId: this.userId, publicKey: EJSON.stringify(this.exportedPublicKey) });
	}

	deny() {
		this.reset();
		TAGT.Notifications.notifyUser(this.peerId, 'otr', 'deny', { roomId: this.roomId, userId: this.userId });
	}

	end() {
		this.reset();
		TAGT.Notifications.notifyUser(this.peerId, 'otr', 'end', { roomId: this.roomId, userId: this.userId });
	}
	...
	generateKeyPair() {
		...
		return TAGT.OTR.crypto.generateKey({
			name: 'ECDH',
			namedCurve: 'P-256'
		}, false, ['deriveKey', 'deriveBits'])
			.then((keyPair) => {
				this.keyPair = keyPair;
				return TAGT.OTR.crypto.exportKey('jwk', keyPair.publicKey);
			})
	...
	importPublicKey(publicKey) {
		return TAGT.OTR.crypto.importKey('jwk', EJSON.parse(publicKey), {
			name: 'ECDH',
			namedCurve: 'P-256'
		}, false, []).then((peerPublicKey) => {
			return TAGT.OTR.crypto.deriveBits({
				name: 'ECDH',
				namedCurve: 'P-256',
				public: peerPublicKey
			}, this.keyPair.privateKey, 256);
		}).then((bits) => {
			return TAGT.OTR.crypto.digest({
				name: 'SHA-256'
			}, bits);
		}).then((hashedBits) => {
			// We truncate the hash to 128 bits.
			var sessionKeyData = new Uint8Array(hashedBits).slice(0, 16);
			return TAGT.OTR.crypto.importKey('raw', sessionKeyData, {
				name: 'AES-GCM'
	...
	encryptText(data) {
		...
		return TAGT.OTR.crypto.encrypt({
			name: 'AES-GCM',
			iv: iv
	...
	decrypt(message) {
		...
		return TAGT.OTR.crypto.decrypt({
			name: 'AES-GCM',
			iv: iv
