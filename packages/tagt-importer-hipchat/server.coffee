Importer.HipChat = class Importer.HipChat extends Importer.Base
			...
			for user in @users.users when user.do_import
				do (user) =>
					Meteor.runAsUser startedByUserId, () =>
						existantUser = TAGT.models.Users.findOneByEmailAddress user.email
						if existantUser
							...
							if user.name?
								TAGT.models.Users.setName userId, user.name
				...
				do (channel) =>
					Meteor.runAsUser startedByUserId, () =>
						...
						if existantRoom
							...
							TAGT.models.Rooms.update { _id: channel.rocketId }, { $set: { 'ts': new Date(channel.created * 1000) }}
						@addCountCompleted 1
			@collection.update { _id: @channels._id }, { $set: { 'channels': @channels.channels }}
				...
				do (channel, messagesObj) =>
					Meteor.runAsUser startedByUserId, () =>
						...
						if hipchatChannel?.do_import
							room = TAGT.models.Rooms.findOneById hipchatChannel.rocketId, { fields: { usernames: 1, t: 1, name: 1 } }
							for date, msgs of messagesObj
								...
								for message in msgs.messages
									if message.from?
										...
										if user?
											msgObj =
												msg: @convertHipChatMessageToTAGT(message.message)
	...
	convertHipChatMessageToTAGT: (message) =>
		if message?
