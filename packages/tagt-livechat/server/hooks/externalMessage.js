...
TAGT.settings.get('Livechat_Knowledge_Enabled', function(key, value) {
	knowledgeEnabled = value;
});
TAGT.settings.get('Livechat_Knowledge_Apiai_Key', function(key, value) {
	apiaiKey = value;
});
TAGT.settings.get('Livechat_Knowledge_Apiai_Language', function(key, value) {
	apiaiLanguage = value;
});
...
TAGT.callbacks.add('afterSaveMessage', function(message, room) {
	...
	Meteor.defer(() => {
		try {
			...
			if (response.data && response.data.status.code === 200 && !_.isEmpty(response.data.result.fulfillment.speech)) {
				TAGT.models.LivechatExternalMessage.insert({
					rid: message.rid,
	...
	return message;
}, TAGT.callbacks.priority.LOW, 'externalWebHook');
