...
TAGT.settings.get('Livechat_forward_open_chats_timeout', function(key, value) {
	forwardChatTimeout = value * 1000;
});

TAGT.settings.get('Livechat_forward_open_chats', function(key, value) {
	monitorAgents = value;
	if (value) {
		if (!agentsHandler) {
			agentsHandler = TAGT.models.Users.findOnlineAgents().observeChanges({
				added(id) {
					onlineAgents.add(id);
				},
				changed(id, fields) {
					if (fields.statusLivechat && fields.statusLivechat === 'not-available') {
						onlineAgents.remove(id, () => {
							TAGT.Livechat.forwardOpenChats(id);
						});
					} else {
						onlineAgents.add(id);
					}
	...
	if (onlineAgents.exists(user._id)) {
		if (statusConnection === 'offline' || user.statusLivechat === 'not-available') {
			onlineAgents.remove(user._id, () => {
				TAGT.Livechat.forwardOpenChats(user._id);
			});
		}
	}
});
