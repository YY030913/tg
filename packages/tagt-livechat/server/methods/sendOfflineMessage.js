...
Meteor.methods({
	'livechat:sendOfflineMessage'(data) {
		...
		let fromEmail = TAGT.settings.get('From_Email').match(/\b[A-Z0-9._%+-]+@(?:[A-Z0-9-]+\.)+[A-Z]{2,4}\b/i);

		if (fromEmail) {
			fromEmail = fromEmail[0];
		} else {
			fromEmail = TAGT.settings.get('From_Email');
		}

		Meteor.defer(() => {
			Email.send({
				to: TAGT.settings.get('Livechat_offline_email'),
		...
		Meteor.defer(() => {
			TAGT.callbacks.run('livechat.offlineMessage', data);
		});
