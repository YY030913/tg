...
Template.livechat.helpers({
	...
	rooms() {
		...
		const user = TAGT.models.Users.findOne(Meteor.userId(), { fields: { 'settings.preferences.unreadRoomsMode': 1 } });
	...
	guestPool() {
		return TAGT.settings.get('Livechat_Routing_Method') === 'Guest_Pool';
	},
	available() {
	...
	showQueueLink() {
		if (TAGT.settings.get('Livechat_Routing_Method') !== 'Least_Amount') {
			return false;
		}
		return TAGT.authz.hasRole(Meteor.userId(), 'livechat-manager') || (Template.instance().statusLivechat.get() === 'available' && TAGT.settings.get('Livechat_show_queue_list_link'));
	},
		...
		swal({
			...
			if (isConfirm) {
				Meteor.call('livechat:takeInquiry', this._id, (error, result) => {
					if (!error) {
						FlowRouter.go(TAGT.roomTypes.getRouteLink(result.t, result));
					}
				});
			}
		});
...
Template.livechat.onCreated(function() {
	...
	this.autorun(() => {
		const user = TAGT.models.Users.findOne(Meteor.userId(), { fields: { statusLivechat: 1 } });
