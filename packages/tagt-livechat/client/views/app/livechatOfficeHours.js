...
Template.livechatOfficeHours.events({
	...
	'submit .rocket-form'(e, instance) {
		...
		TAGT.settings.set('Livechat_enable_office_hours', instance.enableOfficeHours.get(), (err/*, success*/) => {
			if (err) {
				return handleError(err);
			}
...
Template.livechatOfficeHours.onCreated(function() {
	...
	this.autorun(() => {
		this.enableOfficeHours.set(TAGT.settings.get('Livechat_enable_office_hours'));
	});
});