Meteor.methods
	'authorization:removeUserFromRole': (roleName, username, scope) ->
		...
		remove = TAGT.models.Roles.removeUserRoles user._id, roleName, scope

		if TAGT.settings.get('UI_DisplayRoles')
			TAGT.Notifications.notifyAll('roles-change', { type: 'removed', _id: roleName, u: { _id: user._id, username: username }, scope: scope });
