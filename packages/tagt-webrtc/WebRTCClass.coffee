...
class WebRTCTransportClass
	...
	constructor: (@webrtcInstance) ->
		...
		TAGT.Notifications.onRoom @webrtcInstance.room, 'webrtc', (type, data) =>
	...
	startCall: (data) ->
		#@log "startCall", data
		Session.set("videoShow", true)
		TAGT.Notifications.notifyUsersOfRoom @webrtcInstance.room, 'webrtc', 'call',
	...
	joinCall: (data) ->
		...
		if data.monitor is true
			TAGT.Notifications.notifyUser data.to, 'webrtc', 'join',
				from: @webrtcInstance.selfId
				room: @webrtcInstance.room
				media: data.media
				monitor: data.monitor
		else
			TAGT.Notifications.notifyUsersOfRoom @webrtcInstance.room, 'webrtc', 'join',
				from: @webrtcInstance.selfId
				room: @webrtcInstance.room
				media: data.media
				monitor: data.monitor
	...
	sendCandidate: (data) ->
		...
		TAGT.Notifications.notifyUser data.to, 'webrtc', 'candidate', data

	sendDescription: (data) ->
		data.from = @webrtcInstance.selfId
		data.room = @webrtcInstance.room
		@log 'WebRTCTransportClass - sendDescription', data
		TAGT.Notifications.notifyUser data.to, 'webrtc', 'description', data

	sendStatus: (data) ->
		@log 'WebRTCTransportClass - sendStatus', data, @webrtcInstance.room
		data.from = @webrtcInstance.selfId
		TAGT.Notifications.notifyRoom @webrtcInstance.room, 'webrtc', 'status', data
...
class WebRTCClass
	...
	constructor: (@selfId, @room) ->
		...
		servers = TAGT.settings.get("WebRTC_Servers")
		...
		@joinWebrtc = false
		Session.set "mobile-show-webrtc", false
	...
	onRemoteStatus: (data) ->
		if !@joinWebrtc
			@onRemoteCall _.extend arguments[0], {room: Session.get('openedRoom')}
			...
			#if @active isnt true then return#所有人观看
	...
	getPeerConnection: (id) ->
		if ChatRoom.findOne(@room).t == "c" && !@peerConnections[@selfId]? #ChatRoom.findOne(@room)
			peerConnection = new RTCPeerConnection @config

			peerConnection.createdAt = Date.now()
			peerConnection.remoteMedia = {}

			@peerConnections[@selfId] = peerConnection
	...
	getUserMedia: (media, onSuccess, onError=@onError) ->
		...
		getScreen = (audioStream) =>
			if document.cookie.indexOf("tagtscreenshare=chrome") is -1 and not window.tagtscreenshare?
				refresh = ->
					swal
						type: "warning"
					...
					if isConfirm
						if @navigator is 'chrome'
							chrome.webstore.install undefined, refresh, ->
								window.open('https://chrome.google.com/webstore/detail/tagt-screen-share/nocfbnnmjnndkbipkabodnheejiegccf')
								refresh()
						else if @navigator is 'firefox'
							window.open('https://addons.mozilla.org/en-GB/firefox/addon/tagt-screen-share/')
							refresh()
	...
	getLocalUserMedia: (callback) ->
		...
		onSuccess = (stream) =>
			...
			self.localUrl.set URL.createObjectURL(stream)

			self.videoEnabled.set self.media.video is true
			self.audioEnabled.set self.media.audio is true
			...
			item =
				id: self.selfId,
				url: self.localUrl.get(),
				stateText: 'Connected',
				connected: true

			items = self.remoteItems.get()
			items.push item
			
			itemsById = {}
			itemsById[self.d] = item
					
			self.remoteItems.set items
			self.remoteItemsById.set itemsById

		onError = (error) =>
			callback false
			self.onError error
			Meteor.call "leaveWebrtc", Session.get("openedRoom")


		
		Meteor.call "joinWebrtc", Session.get("openedRoom"), Session.get("joinedWebrtcDirection"), (error, success) ->
			if error?
				toastr.error error.message
			else
				self.getUserMedia self.media, onSuccess, onError
	...
	stopPeerConnection: (id) ->
		...
		Session.set("joinedWebrtcDirection", null)
		Session.set "remoteCallData", null
		Meteor.call "leaveWebrtc", Session.get("openedRoom")
	...
	startCall: (media={}) ->
		...
		if Meteor.isCordova || md.mobile()?
			Session.set "mobile-show-webrtc", true
		Meteor.call "clearWebrtc", Session.get("openedRoom")
	...
	###
	onRemoteCall: (data) ->
		if data.room == Session.get("openedRoom")
			Session.set "remoteCallData", data
	...
	joinCallOnlyView: (data={}) ->
		@log "joinCallOnlyView"
		@joinWebrtc = true
		md = new MobileDetect(window.navigator.userAgent);
		if Meteor.isCordova || md.mobile()?
			Session.set "mobile-show-webrtc", true
	...
	canStartUserMedia: ->
		#@log "canStartUserMedia", ChatRoom.findOne(@room)
		if ChatRoom.findOne(@room).t == "c"
			if CaoLiao.models.Debate.findOne(ChatRoom.findOne(@room).did).debateType == "pk" 
				if ChatRoom.findOne(@room).invite.length > 0
					return _.where(ChatRoom.findOne(@room).invite, {id: @selfId}).length > 0
				else
					if !CaoLiao.models.Debate.findOne(ChatRoom.findOne(@room).did).webrtcJoined? || CaoLiao.models.Debate.findOne(ChatRoom.findOne(@room).did).webrtcJoined.length < 2
						return true
					else
						return false

			if CaoLiao.models.Debate.findOne(ChatRoom.findOne(@room).did).debateType == "debate" 
				if ChatRoom.findOne(@room).invite.length > 0
					return _.where(ChatRoom.findOne(@room).invite, {id: @selfId}).length > 0
				else
					if !CaoLiao.models.Debate.findOne(ChatRoom.findOne(@room).did).webrtcJoined? || CaoLiao.models.Debate.findOne(ChatRoom.findOne(@room).did).webrtcJoined.length < 8
						if (!ChatRoom.findOne(@room).left? || ChatRoom.findOne(@room).left.length < 4) && Session.get("joinedWebrtcDirection") == "left"
							return true

						if (!ChatRoom.findOne(@room).right? || ChatRoom.findOne(@room).right.length < 4) && Session.get("joinedWebrtcDirection") == "right"
							return true
		...
		return true #Meteor.userId in ChatRoom.findOne(@room).invite
...
WebRTC = new class
	...
	getInstanceByRoomId: (roomId) ->
		...
		switch subscription.t
			when 'd'
				enabled = TAGT.settings.get('WebRTC_Enable_Direct')
			when 'p'
				enabled = TAGT.settings.get('WebRTC_Enable_Private')
			when 'c'
				enabled = TAGT.settings.get('WebRTC_Enable_Channel')
...
Meteor.startup ->
	Tracker.autorun ->
		if Meteor.userId()
			TAGT.Notifications.onUser 'webrtc', (type, data) =>
