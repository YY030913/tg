...
class IframeLogin {
	constructor() {
		...
		Tracker.autorun((c) => {
			this.enabled = TAGT.settings.get('Accounts_iframe_enabled');
			this.reactiveEnabled.set(this.enabled);

			this.iframeUrl = TAGT.settings.get('Accounts_iframe_url');
			this.apiUrl = TAGT.settings.get('Accounts_Iframe_api_url');
			this.apiMethod = TAGT.settings.get('Accounts_Iframe_api_method');
...
TAGT.iframeLogin = new IframeLogin();
...
window.addEventListener('message', (e) => {
	...
	switch (e.data.event) {
		case 'try-iframe-login':
			TAGT.iframeLogin.tryLogin((error) => {
				if (error) {
		...
		case 'login-with-token':
			TAGT.iframeLogin.loginWithToken(e.data, (error) => {
				if (error) {
					e.source.postMessage({
						event: 'login-error',
						response: error.message
