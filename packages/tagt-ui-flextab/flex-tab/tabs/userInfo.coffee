Template.userInfo.helpers
	...
	canDirectMessage: (username) ->
		return TAGT.authz.hasAllPermission('create-d') and Meteor.user()?.username isnt username
	...
	userTime: ->
		user = Template.instance().user.get()
		if user.utcOffset?
			return Template.instance().now.get().utcOffset(user.utcOffset).format(TAGT.settings.get('Message_TimeFormat'))

	canRemoveUser: ->
		return TAGT.authz.hasAllPermission('remove-user', Session.get('openedRoom'))

	canMuteUser: ->
		return TAGT.authz.hasAllPermission('mute-user', Session.get('openedRoom'))
	...
	canSetModerator: ->
		return TAGT.authz.hasAllPermission('set-moderator', Session.get('openedRoom'))

	isModerator: ->
		return !!RoomRoles.findOne({ rid: Session.get('openedRoom'), "u._id": Template.instance().user.get()?._id, roles: 'moderator' })

	canSetOwner: ->
		return TAGT.authz.hasAllPermission('set-owner', Session.get('openedRoom'))
	...
	hasAdminRole: ->
		return TAGT.authz.hasRole(Template.instance().user.get()?._id, 'admin')
	...
	roleTags: ->
		...
		return TAGT.models.Roles.find({ _id: { $in: roles }, description: { $exists: 1 } }, { fields: { description: 1 } })
...
Template.userInfo.events
	...
	'click .remove-user': (e, instance) ->
		...
		if TAGT.authz.hasAllPermission('remove-user', rid)
			swal {
	...
	'click .mute-user': (e, instance) ->
		...
		rid = Session.get('openedRoom')
		room = ChatRoom.findOne rid
		if TAGT.authz.hasAllPermission('mute-user', rid)
			swal {
				title: t('Are_you_sure')
