Template.membersListPage.helpers
	tAddUsers: ->
		return t('Add_users')

	isGroupChat: ->
		return ChatRoom.findOne(this.rid, { reactive: false })?.t in ['c', 'p', 'b']
	...
	canAddUser: ->
		...
		return roomData.t in ['p', 'c'] and TAGT.authz.hasAllPermission('add-user-to-room', this._id)
	...
	userInfoDetail: ->
		...
		return {
			...
			showAll: room?.t in ['c', 'p', 'b']
			video: room?.t in ['d']
		}

Template.membersListPage.events
	...
	'click .user-card-room': (e, instance) ->
		FlowRouter.go "admin-userInfo", {uid: @_id}

Template.membersListPage.onCreated ->
	#@showAllUsers = new ReactiveVar false
	@showAllUsers = new ReactiveVar true
	...
	@autorun =>
		...
		data.rid = FlowRouter.getParam('rid')
