...
Template.uploadedFilesList.helpers
	...
	canDelete: ->
		return TAGT.authz.hasAtLeastOnePermission('delete-message', @rid) or TAGT.settings.get('Message_AllowDeleting') and @userId is Meteor.userId()
...
Template.uploadedFilesList.events
	...
	'click .icon-trash': (e, t) ->
		...
		swal {
			...
			TAGT.models.Uploads.remove self._id, () ->
				if msg
