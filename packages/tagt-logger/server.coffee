...
@Logger = class Logger
	...
	_getCallerDetails: ->
		...
		for item, index in lines when index > 0
			...
			if not line.match(/packages\/tagt_logger(?:\/|\.js)/)
				break
...
StdOut = new class extends EventEmitter
	constructor: ->
		...
		process.stdout.write = (string, encoding, fd) =>
			...
			if TAGT?.settings?.get('Log_View_Limit')? and @queue.length > TAGT.settings.get('Log_View_Limit')
				@queue.shift()
...
Meteor.publish 'stdout', ->
	...
	if TAGT.authz.hasPermission(@userId, 'view-logs') isnt true
		return @ready()
