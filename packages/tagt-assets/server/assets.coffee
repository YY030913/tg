...
@TAGTAssetsInstance = new TAGTFile.GridFS
	name: 'assets'
...
TAGT.Assets = new class
	mime: mime
	assets: assets

	setAsset: (binaryContent, contentType, asset) ->
		if not assets[asset]?
			throw new Meteor.Error "error-invalid-asset", 'Invalid asset', { function: 'TAGT.Assets.setAsset' }

		extension = mime.extension(contentType)
		if extension not in assets[asset].constraints.extensions
			throw new Meteor.Error contentType, 'Invalid file type: ' + contentType, { function: 'TAGT.Assets.setAsset', errorTitle: 'error-invalid-file-type' }
		...
		rs = TAGTFile.bufferToStream file
		TAGTAssetsInstance.deleteFile asset
		ws = TAGTAssetsInstance.createWriteStream asset, contentType
		ws.on 'end', Meteor.bindEnvironment ->
			Meteor.setTimeout ->
				...
				TAGT.settings.updateById key, value
				TAGT.Assets.processAsset key, value
			, 200
	...
	unsetAsset: (asset) ->
		if not assets[asset]?
			throw new Meteor.Error "error-invalid-asset", 'Invalid asset', { function: 'TAGT.Assets.unsetAsset' }
	...
	processAsset: (settingKey, settingValue) ->
		...
		file = TAGTAssetsInstance.getFileSync assetKey
		if not file
			assetValue.cache = undefined
			return
...
TAGT.settings.addGroup 'Assets'
for key, value of assets
	do (key, value) ->
		TAGT.settings.add "Assets_#{key}", {defaultUrl: value.defaultUrl}, { type: 'asset', group: 'Assets', fileConstraints: value.constraints, i18nLabel: value.label, asset: key, public: true }


TAGT.models.Settings.find().observe
	added: (record) ->
		TAGT.Assets.processAsset record._id, record.value
	changed: (record) ->
		TAGT.Assets.processAsset record._id, record.value
	removed: (record) ->
		TAGT.Assets.processAsset record._id, undefined
...
Meteor.methods
	refreshClients: ->
		...
		hasPermission = TAGT.authz.hasPermission Meteor.userId(), 'manage-assets'
	...
	setAsset: (binaryContent, contentType, asset) ->
		...
		TAGT.Assets.setAsset binaryContent, contentType, asset
		return
