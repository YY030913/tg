...
TAGT.smarsh.generateEml = () => {
	Meteor.defer(() => {
		const smarshMissingEmail = TAGT.settings.get('Smarsh_MissingEmail_Email');
		const timeZone = TAGT.settings.get('Smarsh_Timezone');

		TAGT.models.Rooms.find().forEach((room) => {
			const smarshHistory = TAGT.smarsh.History.findOne({ _id: room._id });
			...
			TAGT.models.Messages.find(query).forEach((message) => {
				rows.push(opentr);
				...
				//The sender
				rows.push(open20td);
				const sender = TAGT.models.Users.findOne({ _id: message.u._id });
				if (data.users.indexOf(sender._id) === -1) {
					data.users.push(sender._id);
				}

				//Get the user's email, can be nothing if it is an unconfigured bot account (like talk get)
				...
				data.msgs++;
				if (message.t) {
					const messageType = TAGT.MessageTypes.getType(message);
					if (messageType) {
			...
			if (rows.length !== 0) {
				...
				TAGT.smarsh.History.upsert({ _id: room._id }, {
					_id: room._id,
					lastRan: date,
					lastResult: result
				});

				TAGT.smarsh.sendEmail({
					body: result,
					subject: `TalkGet, ${data.users.length} Users, ${data.msgs} Messages, ${data.files.length} Files, ${data.time} Minutes, in ${data.room}`,
					files: data.files
				});
			}
		});
