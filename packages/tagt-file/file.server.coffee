...
TAGTFile =
	...
	enabled: undefined
	enable: ->
		TAGTFile.enabled = true
		TAGT.settings.updateOptionsById 'Accounts_AvatarResize', {alert: undefined}
	disable: ->
		TAGTFile.enabled = false
		TAGT.settings.updateOptionsById 'Accounts_AvatarResize', {alert: 'The_image_resize_will_not_work_because_we_can_not_detect_ImageMagick_or_GraphicsMagick_installed_in_your_server'}
	...
	exec 'gm version', Meteor.bindEnvironment (error, stdout, stderr) ->
		...
		exec 'convert -version', Meteor.bindEnvironment (error, stdout, stderr) ->
			if not error? and stdout.indexOf('ImageMagick') > -1
				if TAGTFile.enabled isnt true
					# Enable GM to work with ImageMagick if no GraphicsMagick
					TAGTFile.gm = TAGTFile.gm.subClass({imageMagick: true})
					TAGTFile.enable()

				TAGT.Info.ImageMagick =
					enabled: true
					version: stdout
			else
				if TAGTFile.enabled isnt true
...
TAGTFile.bufferToStream = (buffer) ->
	bufferStream = new stream.PassThrough()
	bufferStream.end buffer
	return bufferStream

TAGTFile.dataURIParse = (dataURI) ->
...
TAGTFile.addPassThrough = (st, fn) ->
	pass = new stream.PassThrough()
	fn pass, st
	return pass


TAGTFile.GridFS = class
	constructor: (config={}) ->
	...
	createWriteStream: (fileName, contentType) ->
		...
		if self.transformWrite?
			ws = TAGTFile.addPassThrough ws, (rs, ws) ->
				file =
					name: self.name
					fileName: fileName
