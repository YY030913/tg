...
function categoryName(category) {
	for (let emojiPackage in TAGT.emoji.packages) {
		if (TAGT.emoji.packages.hasOwnProperty(emojiPackage)) {
			if (TAGT.emoji.packages[emojiPackage].emojiCategories.hasOwnProperty(category)) {
				return TAGT.emoji.packages[emojiPackage].emojiCategories[category];
			}
		}
	}
...
function getEmojisByCategory(category) {
	...
	for (let emojiPackage in TAGT.emoji.packages) {
		if (TAGT.emoji.packages.hasOwnProperty(emojiPackage)) {
			if (TAGT.emoji.packages[emojiPackage].emojisByCategory.hasOwnProperty(category)) {
				let total = TAGT.emoji.packages[emojiPackage].emojisByCategory[category].length;
				for (let i = 0; i < total; i++) {
					let emoji = TAGT.emoji.packages[emojiPackage].emojisByCategory[category][i];
					let tone = '';

					if (actualTone > 0 && TAGT.emoji.packages[emojiPackage].toneList.hasOwnProperty(emoji)) {
					...
					if (isSetNotNull(() => TAGT.emoji.list[`:${emoji}:`].emojiPackage)) {
						let correctPackage = TAGT.emoji.list[`:${emoji}:`].emojiPackage;

						const image = TAGT.emoji.packages[correctPackage].render(`:${emoji}${tone}:`);
...
function getEmojisBySearchTerm(searchTerm) {
	...
	for (let emoji in TAGT.emoji.list) {
		if (!TAGT.emoji.list.hasOwnProperty(emoji)) {
			continue;
		}

		if (searchRegExp.test(emoji)) {
			let emojiObject = TAGT.emoji.list[emoji];
			...
			for (let key in TAGT.emoji.packages[emojiPackage].emojisByCategory) {
				if (TAGT.emoji.packages[emojiPackage].emojisByCategory.hasOwnProperty(key)) {
					let contents = TAGT.emoji.packages[emojiPackage].emojisByCategory[key];
			...
			if (emojiFound) {
				let image = TAGT.emoji.packages[emojiPackage].render(`:${emoji}${tone}:`);
				html += `<li class="emoji-${emoji}" data-emoji="${emoji}" title="${emoji}">${image}</li>`;
			}
		}
	}
...
Template.emojiPicker.helpers({
	...
	emojiByCategory(category) {
		...
		for (let emojiPackage in TAGT.emoji.packages) {
			if (TAGT.emoji.packages.hasOwnProperty(emojiPackage)) {
				if (TAGT.emoji.packages[emojiPackage].emojisByCategory.hasOwnProperty(category)) {
					emojisByCategory = emojisByCategory.concat(TAGT.emoji.packages[emojiPackage].emojisByCategory[category]);
				}
			}
		}
...
Template.emojiPicker.events({
	...
	'click .tone-selector .tone'(event, instance) {
		...
		for (let emojiPackage in TAGT.emoji.packages) {
			if (TAGT.emoji.packages.hasOwnProperty(emojiPackage)) {
				if (TAGT.emoji.packages[emojiPackage].hasOwnProperty('toneList')) {
					for (let emoji in TAGT.emoji.packages[emojiPackage].toneList) {
						if (TAGT.emoji.packages[emojiPackage].toneList.hasOwnProperty(emoji)) {
							$('.emoji-'+emoji).html(TAGT.emoji.packages[emojiPackage].render(':' + emoji + newTone + ':'));
						}
					}
				}
			}
	...
	'click .emoji-list li'(event, instance) {
		...
		TAGT.EmojiPicker.pickEmoji(emoji + tone);
	},
...
Template.emojiPicker.onCreated(function() {
	this.tone = TAGT.EmojiPicker.getTone();
	let recent = TAGT.EmojiPicker.getRecent();

	this.currentCategory = new ReactiveVar(recent.length > 0 ? 'recent' : 'people');
	this.currentSearchTerm = new ReactiveVar('');

	recent.forEach((emoji) => {
		TAGT.emoji.packages.base.emojisByCategory.recent.push(emoji);
	});
