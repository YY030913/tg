 ...
 */
TAGT.callbacks.add('renderMessage', (message) => {
	if (isSetNotNull(() => Meteor.user().settings.preferences.useEmojis) && !Meteor.user().settings.preferences.useEmojis) {
		return message;
	}

	if (_.trim(message.html)) {
		Object.keys(TAGT.emoji.packages).forEach((emojiPackage) => {
			message.html = TAGT.emoji.packages[emojiPackage].render(message.html);
		});
	...
	return message;
}, TAGT.callbacks.priority.LOW, 'emoji');
