Template.channelSettingsPage.helpers
	canEdit: ->
		return false
		# return TAGT.authz.hasAllPermission('edit-room', @rid)
	canArchiveOrUnarchive: ->
		return TAGT.authz.hasAtLeastOnePermission(['archive-room', 'unarchive-room'], @rid)
	editing: (field) ->
...
Template.channelSettingsPage.events
...
Template.channelSettingsPage.onCreated ->
	@editing = new ReactiveVar
	@data.rid = FlowRouter.current().params.rid
	#new ReactiveVar 
	#@data.rid.set(FlowRouter.current().params.rid)
	...
	@validateRoomName = =>
		...
		if not TAGT.authz.hasAllPermission('edit-room', rid) or room.t not in ['c', 'p']
		...
		if name.length > TAGT.settings.get('UTF8_Names_MaxLength') || name.length < TAGT.settings.get('UTF8_Names_MinLength')
			throw new Meteor.Error 'error-invalid-room-name-length', "#{name} is not a valid length must longer then #{TAGT.settings.get('UTF8_Names_MinLength')} and litter then #{TAGT.settings.get('UTF8_Names_MaxLength')}"
