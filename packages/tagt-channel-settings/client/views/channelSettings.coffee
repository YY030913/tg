Template.channelSettings.helpers
	...
	channelSettings: ->
		return TAGT.ChannelSettings.getOptions()
	...
	canDeleteRoom: ->
		...
		return roomType? and TAGT.authz.hasAtLeastOnePermission("delete-#{roomType}", @rid)
	readOnly: ->
...
Template.channelSettings.onCreated ->
	...
	@settings =
		name:
			...
			canEdit: (room) => TAGT.authz.hasAllPermission('edit-room', room._id)
			save: (value, room) ->
				if not TAGT.authz.hasAllPermission('edit-room', room._id) or room.t not in ['c', 'p']
				...
				TAGT.callbacks.run 'roomNameChanged', { _id: room._id, name: value }
		...
		topic:
			...
			save: (value, room) ->
				Meteor.call 'saveRoomSettings', room._id, 'roomTopic', value, (err, result) ->
					...
					TAGT.callbacks.run 'roomTopicChanged', room
		...
		t:
			...
			save: (value, room) ->
				...
				TAGT.callbacks.run 'roomTypeChanged', room
		...
		ro:
			...
			label: 'Read_only'
			canView: (room) => room.t isnt 'd'
			canEdit: (room) => TAGT.authz.hasAllPermission('set-readonly', room._id)
			save: (value, room) ->
