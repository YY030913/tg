...
function AutoLinker(message) {
	if (_.trim(message.html)) {
		const regUrls = new RegExp(TAGT.settings.get('AutoLinker_UrlsRegExp'));

		const autolinker = new Autolinker({
			stripPrefix: TAGT.settings.get('AutoLinker_StripPrefix'),
			urls: {
				schemeMatches: TAGT.settings.get('AutoLinker_Urls_Scheme'),
				wwwMatches: TAGT.settings.get('AutoLinker_Urls_www'),
				tldMatches: TAGT.settings.get('AutoLinker_Urls_TLD')
			},
			email: TAGT.settings.get('AutoLinker_Email'),
			phone: TAGT.settings.get('AutoLinker_Phone'),
			twitter: false,
			replaceFn(match) {
		...
		if (TAGT.settings.get('Katex_Enabled')) {
			regNonAutoLink = /(```\w*[\n ]?[\s\S]*?```+?)|(`(?:[^`]+)`)|(\\\(\w*[\n ]?[\s\S]*?\\\)+?)/;
		}
...
TAGT.callbacks.add('renderMessage', AutoLinker);
