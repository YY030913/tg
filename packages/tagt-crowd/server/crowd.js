...
const CROWD = class CROWD {
	constructor() {
		...
		let url = TAGT.settings.get('CROWD_URL');
		...
		this.options = {
			...
			application: {
				name: TAGT.settings.get('CROWD_APP_USERNAME'),
				password: TAGT.settings.get('CROWD_APP_PASSWORD')
			},
			rejectUnauthorized: TAGT.settings.get('CROWD_Reject_Unauthorized')
		};
	...
	sync() {
		if (TAGT.settings.get('CROWD_Enable') !== true) {
			return;
		}
		...
		const users = TAGT.models.Users.findCrowdUsers();
		if (users) {
...
TAGT.settings.get('CROWD_Sync_User_Data', function(key, value) {
