...
TAGT.CachedCollectionManager = new CachedCollectionManager;
...
class CachedCollection {
	constructor({
		...
		TAGT.CachedCollectionManager.register(this);

		if (initOnLogin === true) {
			TAGT.CachedCollectionManager.onLogin(() => {
	...
	sync() {
		if (TAGT.CachedCollectionManager.syncEnabled === false || Meteor.connection._outstandingMethodBlocks.length !== 0) {
			return false;
		}
	...
	setupListener(eventType, eventName) {
		TAGT.Notifications[eventType || this.eventType](eventName || this.eventName, (t, record) => {
...
TAGT.CachedCollection = CachedCollection;
