Meteor.methods
	sendMessage: (message) ->
		...
		if message.msg?.length > TAGT.settings.get('Message_MaxAllowedSize')
		...
		user = TAGT.models.Users.findOneById Meteor.userId(), fields: username: 1, name: 1
		...
		message.alias = user.name if not message.alias? and TAGT.settings.get 'Message_SetNameToAliasEnabled'
