TAGT.callbacks.add('beforeSaveMessage', function(message) {
	...
	if (message.mentions != null &&
		...
		if (!TAGT.authz.hasPermission(message.u._id, 'mention-all')) {

			// Get the language of the user for the error notification.
			let language = TAGT.models.Users.findOneById(message.u._id).language;
			let action = TAPi18n.__('Notify_all_in_this_room', {}, language);

			// Add a notification to the chat, informing the user that this
			// action is not allowed.
			TAGT.Notifications.notifyUser(message.u._id, 'message', {
				_id: Random.id(),
				rid: message.rid,
				ts: new Date,
