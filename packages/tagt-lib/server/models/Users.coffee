TAGT.models.Users = new class extends TAGT.models._Base
	constructor: ->
		@model = Meteor.users
	...
	addDebate: (_id, debate) ->
		debates = [].concat debate
		update =
			$addToSet: {
				debates: { $each: debates }
			}

		return @update _id, update


	addCreateDebate: (_id, debate) ->
		debates = [].concat debate
		update =
			$addToSet: {
				createDebates: { $each: debates }
			}
