TAGT.saveCustomFields = function(userId, formData) {
	if (s.trim(TAGT.settings.get('Accounts_CustomFields')) !== '') {
		let customFieldsMeta;
		try {
			customFieldsMeta = JSON.parse(TAGT.settings.get('Accounts_CustomFields'));
		} catch (e) {
		...
		TAGT.models.Users.setCustomFields(userId, customFields);
		...
		Object.keys(customFields).forEach((fieldName) => {
			...
			TAGT.models.Users.update(userId, update);
		});
