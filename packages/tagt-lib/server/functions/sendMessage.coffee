TAGT.sendMessage = (user, message, room, upsert = false) ->
	...
	if not room.usernames?
		room = TAGT.models.Rooms.findOneById(room._id)
	...
	if message._id? and upsert
		_id = message._id
		delete message._id
		TAGT.models.Messages.upsert {_id: _id, 'u._id': message.u._id}, message
		message._id = _id
	else
		message._id = TAGT.models.Messages.insert message
	...
	Meteor.defer ->
		...
		TAGT.callbacks.run 'afterSaveMessage', message, room
