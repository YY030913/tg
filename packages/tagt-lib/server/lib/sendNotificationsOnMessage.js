...
TAGT.callbacks.add('afterSaveMessage', function(message, room) {
	...
	var user = TAGT.models.Users.findOneById(message.u._id);
	...
	TAGT.models.Subscriptions.findNotificationPreferencesByRoom(room._id).forEach(function(subscription) {
	...
	userIdsToPushNotify = [];
	usersWithHighlights = [];
	highlights = TAGT.models.Users.findUsersByUsernamesWithHighlights(room.usernames, { fields: { '_id': 1, 'settings.preferences.highlights': 1 }}).fetch();
	...
	if (TAGT.settings.get('Push_show_message')) {
	...
	let push_username;
	let push_room;
	if (TAGT.settings.get('Push_show_username_room')) {
	...
	if ((room.t == null) || room.t === 'd') {
		...
		userOfMention = TAGT.models.Users.findOne({
			_id: userOfMentionId
		}, {
			fields: {
				username: 1,
		...
		if (userOfMention != null) {
			TAGT.Sandstorm.notify(message, [userOfMention._id],
				'@' + user.username + ': ' + message.msg, 'privateMessage');

		}
		if ((userOfMention != null) && canBeNotified(userOfMentionId, 'mobile')) {
			TAGT.Notifications.notifyUser(userOfMention._id, 'notification', {
				title: '@' + user.username,
				text: message.msg,
		...
		if (mentionIds.length > 0 || settings.alwaysNotifyDesktopUsers.length > 0) {
			...
			usersOfDesktopMentions = TAGT.models.Users.find({
				_id: {
		...
		if (mentionIds.length > 0 || settings.alwaysNotifyMobileUsers.length > 0) {
			...
			usersOfMobileMentions = TAGT.models.Users.find({
				_id: {
					$in: mobileMentionIds
				}
			}, {
		...
		if (userIdsToNotify.length > 0) {
			for (j = 0, len1 = userIdsToNotify.length; j < len1; j++) {
				...
				TAGT.Notifications.notifyUser(usersOfMentionId, 'notification', {
					title: title,
					text: message.msg,
		...
		if (room.t === 'p') {
			TAGT.Sandstorm.notify(message, allUserIdsToNotify,
				'@' + user.username + ': ' + message.msg, 'privateMessage');
		} else {
			TAGT.Sandstorm.notify(message, allUserIdsToNotify,
...
}, TAGT.callbacks.priority.LOW, 'sendNotificationOnMessage');
