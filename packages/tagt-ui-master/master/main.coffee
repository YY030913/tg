...
Template.body.onRendered ->
	...
	Tracker.autorun (c) ->
		...
		s = 'script'
		l = 'dataLayer'
		i = TAGT.settings.get 'GoogleTagManager_id'
	...
	Tracker.autorun (c) ->
		if TAGT.settings.get 'Meta_language'
			c.stop()

			Meta.set
				name: 'http-equiv'
				property: 'content-language'
				content: TAGT.settings.get 'Meta_language'
			Meta.set
				name: 'name'
				property: 'language'
				content: TAGT.settings.get 'Meta_language'
	...
	Tracker.autorun (c) ->
		if TAGT.settings.get 'Meta_fb_app_id'
			c.stop()

			Meta.set
				name: 'property'
				property: 'fb:app_id'
				content: TAGT.settings.get 'Meta_fb_app_id'

	Tracker.autorun (c) ->
		if TAGT.settings.get 'Meta_robots'
			c.stop()

			Meta.set
				name: 'name'
				property: 'robots'
				content: TAGT.settings.get 'Meta_robots'

	Tracker.autorun (c) ->
		if TAGT.settings.get 'Meta_google-site-verification'
			c.stop()

			Meta.set
				name: 'name'
				property: 'google-site-verification'
				content: TAGT.settings.get 'Meta_google-site-verification'

	Tracker.autorun (c) ->
		if TAGT.settings.get 'Meta_msvalidate01'
			c.stop()

			Meta.set
				name: 'name'
				property: 'msvalidate.01'
				content: TAGT.settings.get 'Meta_msvalidate01'
...
Template.main.helpers
	...
	siteName: ->
		return TAGT.settings.get 'Site_Name'
	...
	useIframe: ->
		return TAGT.iframeLogin.reactiveEnabled.get()

	iframeUrl: ->
		return TAGT.iframeLogin.reactiveIframeUrl.get()
	...
	flexOpened: ->
		...
		return 'flex-opened' if TAGT.TabBar.isFlexOpen()
	...
	CustomScriptLoggedOut: ->
		TAGT.settings.get 'Custom_Script_Logged_Out'

	CustomScriptLoggedIn: ->
		TAGT.settings.get 'Custom_Script_Logged_In'

	embeddedVersion: ->
		return 'embedded-view' if TAGT.Layout.isEmbedded()
	...
	Tracker.autorun ->
		...
		if prefs?.hideUsernames
			...
			$(document.body).on('mouseenter', 'button.thumb', (e) ->
				...
				if username
					e.stopPropagation()
					TAGT.tooltip.showElement($('<span>').text(username), avatarElem)
			)
		else
