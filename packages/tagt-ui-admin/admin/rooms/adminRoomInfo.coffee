Template.adminRoomInfo.helpers
	...
	canDeleteRoom: ->
		roomType = ChatRoom.findOne(@rid, { fields: { t: 1 }})?.t
		return roomType? and TAGT.authz.hasAtLeastOnePermission("delete-#{roomType}")
	readOnly: ->
		...
		switch @editing.get()
			when 'roomName'
				if @validateRoomName(rid)
					TAGT.callbacks.run 'roomNameChanged', ChatRoom.findOne(rid)
			...
			when 'roomTopic'
				if @validateRoomTopic(rid)
					Meteor.call 'saveRoomSettings', rid, 'roomTopic', @$('input[name=roomTopic]').val(), (err, result) ->
						...
						TAGT.callbacks.run 'roomTopicChanged', ChatRoom.findOne(rid)
			when 'roomType'
				if @validateRoomType(rid)
					TAGT.callbacks.run 'roomTypeChanged', ChatRoom.findOne(rid)
				...
				if @$('input[name=archivationState]:checked').val() is 'true'
					if ChatRoom.findOne(rid)?.archived isnt true
						Meteor.call 'archiveRoom', rid, (err, results) ->
							...
							TAGT.callbacks.run 'archiveRoom', ChatRoom.findOne(rid)
				else
					if ChatRoom.findOne(rid)?.archived is true
						Meteor.call 'unarchiveRoom', rid, (err, results) ->
							...
							TAGT.callbacks.run 'unarchiveRoom', ChatRoom.findOne(rid)
			when 'readOnly'
