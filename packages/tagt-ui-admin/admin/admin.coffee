...
Template.admin.onCreated ->
	...
	TAGT.settings.collectionPrivate.find().observe
		added: (data) ->
...
Template.admin.helpers
	...
	appLanguage: (key) ->
		if !key
			return !TAGT.settings.get('Language')
		selected = (TAGT.settings.get('Language'))?.split('-').shift().toLowerCase() is key
		return selected
	...
	selectedOption: (_id, val) ->
		return TAGT.settings.collectionPrivate.findOne({_id: _id})?.value is val
		...
		Meteor.defer ->
			...
			onChange = ->
				...
				TempSettings.update {_id: _id},
					$set:
						value: value
						changed: TAGT.settings.collectionPrivate.findOne(_id).value isnt value
...
Template.admin.onRendered ->
	...
	Meteor.setTimeout ->
		$('input.minicolors').minicolors({theme: 'tagt'})
	, 1000
