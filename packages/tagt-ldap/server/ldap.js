...
LDAP = class LDAP {
	constructor() {
		...
		self.options = {
			host: TAGT.settings.get('LDAP_Host'),
			port: TAGT.settings.get('LDAP_Port'),
			encryption: TAGT.settings.get('LDAP_Encryption'),
			ca_cert: TAGT.settings.get('LDAP_CA_Cert'),
			reject_unauthorized: TAGT.settings.get('LDAP_Reject_Unauthorized') || false,
			domain_base: TAGT.settings.get('LDAP_Domain_Base'),
			use_custom_domain_search: TAGT.settings.get('LDAP_Use_Custom_Domain_Search'),
			custom_domain_search: TAGT.settings.get('LDAP_Custom_Domain_Search'),
			domain_search_user: TAGT.settings.get('LDAP_Domain_Search_User'),
			domain_search_password: TAGT.settings.get('LDAP_Domain_Search_Password'),
			domain_search_filter: TAGT.settings.get('LDAP_Domain_Search_Filter'),
			domain_search_user_id: TAGT.settings.get('LDAP_Domain_Search_User_ID'),
			domain_search_object_class: TAGT.settings.get('LDAP_Domain_Search_Object_Class'),
			domain_search_object_category: TAGT.settings.get('LDAP_Domain_Search_Object_Category')
		};
	...
	connectAsync(callback) {
		...
		if (self.options.ca_cert && self.options.ca_cert !== '') {
			...
			var chainLines = TAGT.settings.get('LDAP_CA_Cert').split('\n');
			var cert = [];
			var ca = [];
		...
		if (self.options.encryption === 'tls') {
			...
			// https://github.com/TAGT/TalkGet/issues/2035
	...
	getUserByIdSync(id, attribute) {
		...
		let Unique_Identifier_Field = TAGT.settings.get('LDAP_Unique_Identifier_Field').split(',');
