...
slug = function slug(text) {
	if (TAGT.settings.get('UTF8_Names_Slugify') !== true) {
		return text;
	}
	text = slugify(text, '.');
...
getLdapUsername = function getLdapUsername(ldapUser) {
	let usernameField = TAGT.settings.get('LDAP_Username_Field');
...
getLdapUserUniqueID = function getLdapUserUniqueID(ldapUser) {
	let Unique_Identifier_Field = TAGT.settings.get('LDAP_Unique_Identifier_Field');
	...
	let LDAP_Domain_Search_User_ID = TAGT.settings.get('LDAP_Domain_Search_User_ID');
...
getDataToSyncUserData = function getDataToSyncUserData(ldapUser, user) {
	const syncUserData = TAGT.settings.get('LDAP_Sync_User_Data');
	const syncUserDataFieldMap = TAGT.settings.get('LDAP_Sync_User_Data_FieldMap').trim();
...
syncUserData = function syncUserData(user, ldapUser) {
	...
	if (TAGT.settings.get('LDAP_Username_Field') !== '') {
		const username = slug(getLdapUsername(ldapUser));
		if (user && user._id && username !== user.username) {
			logger.info('Syncing user username', user.username, '->', username);
			TAGT._setUsername(user._id, username);
		}
	}

	if (user && user._id && TAGT.settings.get('LDAP_Sync_User_Avatar') === true) {
		const avatar = ldapUser.raw.thumbnailPhoto || ldapUser.raw.jpegPhoto;
		if (avatar) {
			logger.info('Syncing user avatar');
			const rs = TAGTFile.bufferToStream(avatar);
			TAGTFileAvatarInstance.deleteFile(encodeURIComponent(`${user.username}.jpg`));
			const ws = TAGTFileAvatarInstance.createWriteStream(encodeURIComponent(`${user.username}.jpg`), 'image/jpeg');
			ws.on('end', Meteor.bindEnvironment(function() {
				Meteor.setTimeout(function() {
					TAGT.models.Users.setAvatarOrigin(user._id, 'ldap');
...
addLdapUser = function addLdapUser(ldapUser, username, password) {
	...
	} else if (TAGT.settings.get('LDAP_Default_Domain') !== '') {
		userObject.email = username + '@' + TAGT.settings.get('LDAP_Default_Domain');
	} else {
...
sync = function sync() {
	...
	try {
		...
		const users = TAGT.models.Users.findLDAPUsers();

		if (TAGT.settings.get('LDAP_Import_Users') === true && TAGT.settings.get('LDAP_Username_Field') !== '') {
...
TAGT.settings.get('LDAP_Sync_User_Data', function(key, value) {
