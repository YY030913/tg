...
class SlackBridge {
	constructor() {
		...
		this.apiToken = TAGT.settings.get('SlackBridge_APIToken');
		this.aliasFormat = TAGT.settings.get('SlackBridge_AliasFormat');
		this.excludeBotnames = TAGT.settings.get('SlackBridge_Botnames');
		this.rtm = {};
		this.connected = false;
		this.userTags = {};
		this.channelMap = {};

		TAGT.settings.onload('SlackBridge_APIToken', (key, value) => {
			this.apiToken = value;
			if (this.connected) {
				this.disconnect();
				this.connect();
			} else if (TAGT.settings.get('SlackBridge_Enabled')) {
				this.connect();
			}
		});
		...
		TAGT.settings.onload('SlackBridge_AliasFormat', (key, value) => {
			this.aliasFormat = value;
		});

		TAGT.settings.onload('SlackBridge_ExcludeBotnames', (key, value) => {
	...
	convertSlackMessageToTAGT(message) {
	...
	findChannel(channelId) {
		logger.class.debug('Searching for TalkGet channel', channelId);
		return TAGT.models.Rooms.findOneByImportId(channelId);
	}
	...
	addChannel(channelId, hasRetried = false) {
		...
		if (data && data.data && data.data.ok === true) {
			...
			let existingRoom = TAGT.models.Rooms.findOneByName(channelData.name);
			...
			if (existingRoom || channelData.is_general) {
				...
				TAGT.models.Rooms.update({ _id: channelData.rocketId }, { $addToSet: { importIds: channelData.id } });
			} else {
				let users = [];
				...
				try {
					let channel = TAGT.createRoom(isGroup ? 'p' : 'c', channelData.name, creator.username, users);
				...
				TAGT.models.Rooms.update({ _id: channelData.rocketId }, { $set: roomUpdate, $addToSet: { importIds: channelData.id } });
				this.channelMap[channelData.rocketId] = { id: channelId, family: channelId.charAt(0) === 'C' ? 'channels' : 'groups' };
			}
			return TAGT.models.Rooms.findOne(channelData.rocketId);
		}
	...
	findUser(userId) {
		logger.class.debug('Searching for TalkGet user', userId);
		let user = TAGT.models.Users.findOneByImportId(userId);
	...
	addUser(userId) {
		...
		if (data && data.data && data.data.ok === true && data.data.user && data.data.user.profile && data.data.user.profile.email) {
			...
			let existingUser = TAGT.models.Users.findOneByEmailAddress(userData.profile.email) || TAGT.models.Users.findOneByUsername(userData.name);
			if (existingUser) {
				...
				let userUpdate = {
					...
					utcOffset: userData.tz_offset / 3600, // Slack's is -18000 which translates to TalkGet's after dividing by 3600,
					roles: [ 'user' ]
				};
				...
				TAGT.models.Users.update({ _id: userData.rocketId }, { $set: userUpdate });

				let user = TAGT.models.Users.findOneById(userData.rocketId);
				...
				try {
					TAGT.setUserAvatar(user, url, null, 'url');
				} catch (error) {
					logger.class.debug('Error setting user avatar', error.message);
				}
				TAGT.addUserToDefaultChannels(user, true);
			}

			TAGT.models.Users.update({ _id: userData.rocketId }, { $addToSet: { importIds: userData.id } });
			if (!this.userTags[userId]) {
				this.userTags[userId] = { slack: `<@${userId}>`, rocket: `@${userData.name}` };
			}
			logger.class.debug('User: ', userData.rocketId);
			return TAGT.models.Users.findOneById(userData.rocketId);
		}
	...
	sendMessage(room, user, message, msgDataDefaults, importing) {
		if (message.type === 'message') {
			...
			if (!_.isEmpty(message.subtype)) {
				...
				msgObj = {
					msg: this.convertSlackMessageToTAGT(message.text),
					rid: room._id,
					u: {
						_id: user._id,
			...
			if (message.subtype === 'bot_message') {
				user = TAGT.models.Users.findOneById('talk get', { fields: { username: 1 } });
			}
			...
			TAGT.sendMessage(user, msgObj, room, true);
		}
	}
		...
		switch (message.subtype) {
			...
			case 'me_message':
				return this.addAlias(user.username, {
					msg: `_${this.convertSlackMessageToTAGT(message.text)}_`
				});
			...
			case 'message_deleted':
				if (message.previous_message) {
					...
					msgObj = TAGT.models.Messages.findOneById(_id);
					if (msgObj) {
						TAGT.deleteMessage(msgObj, user);
					}
				}
				return;
			case 'channel_join':
				if (importing) {
					TAGT.models.Messages.createUserJoinWithRoomIdAndUser(room._id, user, { ts: new Date(parseInt(message.ts.split('.')[0]) * 1000), imported: 'slackbridge' });
				} else {
					TAGT.addUserToRoom(room._id, user);
				}
				return;
			case 'group_join':
				if (message.inviter) {
					...
					if (importing) {
						TAGT.models.Messages.createUserAddedWithRoomIdAndUser(room._id, user, {
							ts: new Date(parseInt(message.ts.split('.')[0]) * 1000),
							u: {
								_id: inviter._id,
						...
						TAGT.addUserToRoom(room._id, user, inviter);
					}
				}
				return;
			case 'channel_leave':
			case 'group_leave':
				if (importing) {
					TAGT.models.Messages.createUserLeaveWithRoomIdAndUser(room._id, user, {
						ts: new Date(parseInt(message.ts.split('.')[0]) * 1000),
						imported: 'slackbridge'
					});
				} else {
					TAGT.removeUserFromRoom(room._id, user);
				}
				return;
			case 'channel_topic':
			case 'group_topic':
				if (importing) {
					TAGT.models.Messages.createRoomSettingsChangedWithTypeRoomIdMessageAndUser('room_changed_topic', room._id, message.topic, user, { ts: new Date(parseInt(message.ts.split('.')[0]) * 1000), imported: 'slackbridge' });
				} else {
					TAGT.saveRoomTopic(room._id, message.topic, user);
				}
				return;
			case 'channel_purpose':
			case 'group_purpose':
				if (importing) {
					TAGT.models.Messages.createRoomSettingsChangedWithTypeRoomIdMessageAndUser('room_changed_topic', room._id, message.purpose, user, { ts: new Date(parseInt(message.ts.split('.')[0]) * 1000), imported: 'slackbridge' });
				} else {
					TAGT.saveRoomTopic(room._id, message.purpose, user);
				}
				return;
			case 'channel_name':
			case 'group_name':
				if (importing) {
					TAGT.models.Messages.createRoomRenamedWithRoomIdRoomNameAndUser(room._id, message.name, user, { ts: new Date(parseInt(message.ts.split('.')[0]) * 1000), imported: 'slackbridge' });
				} else {
					TAGT.saveRoomName(room._id, message.name, user);
				}
				return;
			case 'channel_archive':
			case 'group_archive':
				if (!importing) {
					TAGT.archiveRoom(room);
				}
				return;
			case 'channel_unarchive':
			case 'group_unarchive':
				if (!importing) {
					TAGT.unarchiveRoom(room);
				}
				return;
			case 'file_share':
			...
			case 'pinned_item':
				if (message.attachments && message.attachments[0] && message.attachments[0].text) {
					msgObj = {
						...
						attachments: [{
							'text' : this.convertSlackMessageToTAGT(message.attachments[0].text),
					...
					if (!importing) {
						TAGT.models.Messages.setPinnedByIdAndUserId(`slack-${message.attachments[0].channel_id}-${message.attachments[0].ts.replace(/\./g, '-')}`, msgObj.u, true, new Date(parseInt(message.ts.split('.')[0]) * 1000));
					}
	...
	**/
	editMessage(room, user, message) {
		let msgObj = {
			...
			rid: room._id,
			msg: this.convertSlackMessageToTAGT(message.message.text)
		};

		TAGT.updateMessage(msgObj, user);
	}
	...
	@param [Object] user the TalkGet user
	@param [Object] room the TalkGet room
	...
	**/
			...
			if (fileId) {
				Meteor.fileStore.write(stream, fileId, (err, file) => {
					...
					if (err) {
						...
						return TAGT.sendMessage(user, msg, room, true);
					}
				});
			}
		}));
	...
	copyChannelInfo(rid, channelMap) {
		logger.class.debug('Copying users from Slack channel to TalkGet', channelMap.id, rid);
		...
		if (response && response.data) {
			...
			if (data && _.isArray(data.members) && data.members.length > 0) {
				for (let member of data.members) {
					...
					if (user) {
						...
						TAGT.addUserToRoom(rid, user, null, true);
					}
				}
			}
			...
			if (topic) {
				...
				TAGT.saveRoomTopic(rid, topic, creator);
			}
		}
	}
			...
			for (let pin of response.data.items) {
				if (pin.message) {
					...
					let msgObj = {
						...
						attachments: [{
							'text' : this.convertSlackMessageToTAGT(pin.message.text),
					...
					TAGT.models.Messages.setPinnedByIdAndUserId(`slack-${pin.channel}-${pin.message.ts.replace(/\./g, '-')}`, msgObj.u, true, new Date(parseInt(pin.message.ts.split('.')[0]) * 1000));
				}
			}
		}
	}
	...
	importMessages(rid, callback) {
		...
		let tagt_room = TAGT.models.Rooms.findOneById(rid);
		if (tagt_room) {
			if (this.channelMap[rid]) {
				this.copyChannelInfo(rid, this.channelMap[rid]);

				logger.class.debug('Importing messages from Slack to TalkGet', this.channelMap[rid], rid);
				...
				logger.class.debug('Pinning Slack channel messages to TalkGet', this.channelMap[rid], rid);
				this.copyPins(rid, this.channelMap[rid]);

				return callback();
			} else {
				let slack_room = this.findSlackChannel(tagt_room.name);
				if (slack_room) {
					...
					logger.class.error('Could not find Slack room with specified name', tagt_room.name);
					return callback(new Meteor.Error('error-slack-room-not-found', 'Could not find Slack room with specified name'));
				}
			}
		} else {
			logger.class.error('Could not find TalkGet room with specified id', rid);
			return callback(new Meteor.Error('error-invalid-room', 'Invalid room'));
		}
	}
}

TAGT.SlackBridge = new SlackBridge;
