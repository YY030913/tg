...
class TAGTAdapter extends Hubot.Adapter
	...
	send: (envelope, strings...) ->
		console.log 'TALKGETCHATADAPTER -> send'.blue if DEBUG
		...
		sendHelper @robot, envelope, strings, (string) =>
			...
			TAGT.sendMessage InternalHubot.user, { msg: string }, { _id: envelope.room }
	...
	emote: (envelope, strings...) ->
		console.log 'TALKGETCHATADAPTER -> emote'.blue if DEBUG
		sendHelper @robot, envelope, strings, (string) =>
	...
	priv: (envelope, strings...) ->
		console.log 'TALKGETCHATADAPTER -> priv'.blue if DEBUG
		sendHelper @robot, envelope, strings, (string) ->
	...
	reply: (envelope, strings...) ->
		console.log 'TALKGETCHATADAPTER -> reply'.blue if DEBUG
	...
	topic: (envelope, strings...) ->
		console.log 'TALKGETCHATADAPTER -> topic'.blue if DEBUG
	...
	play: (envelope, strings...) ->
		console.log 'TALKGETCHATADAPTER -> play'.blue if DEBUG

	# Public: Raw method for invoking the bot to run. Extend this.
	#
	# Returns nothing.
	run: ->
		console.log 'TALKGETCHATADAPTER -> run'.blue if DEBUG
	...
	constructor: (robot) ->
		...
		for modulePath in modulesToLoad
			try
				...
				robot.parseHelp __meteor_bootstrap__.serverDir+'/npm/tagt_internal-hubot/node_modules/'+modulePath
				console.log "Loaded #{modulePath}".green
			catch e
				console.log "can't load #{modulePath}".red
				console.log e

		scriptsToLoad = TAGT.settings.get('InternalHubot_ScriptsToLoad').split(',') or []
		...
		for scriptFile in scriptsToLoad
			try
				...
				# robot.loadFile __meteor_bootstrap__.serverDir+'/npm/node_modules/meteor/tagt_internal-hubot/node_modules/hubot-scripts/src/scripts', scriptFile
				robot.parseHelp __meteor_bootstrap__.serverDir+'/npm/node_modules/meteor/tagt_internal-hubot/node_modules/hubot-scripts/src/scripts/'+scriptFile
	...
	while strings.length > 0
		...
		if typeof(string) == 'function'
			...
			try
				...
				robot.logger.error "TAGT send error: #{err}"

InternalHubot = {}

init = =>
	InternalHubot = new Robot null, null, false, TAGT.settings.get 'InternalHubot_Username'
	InternalHubot.alias = 'bot'
	InternalHubot.adapter = new TAGTAdapter InternalHubot
	HubotScripts(InternalHubot)
	InternalHubot.run()

	if TAGT.settings.get 'InternalHubot_Enabled'
		TAGT.callbacks.add 'afterSaveMessage', InternalHubotReceiver, TAGT.callbacks.priority.LOW, 'InternalHubot'
	else
		TAGT.callbacks.remove 'afterSaveMessage', 'InternalHubot'

Meteor.startup ->
	init()
	TAGT.models.Settings.findByIds([ 'InternalHubot_Username', 'InternalHubot_Enabled', 'InternalHubot_ScriptsToLoad']).observe
		changed: ->
			init()
