Meteor.methods
	'mailMessages': (data) ->
		...
		unless TAGT.authz.hasPermission(Meteor.userId(), 'mail-messages')
		...
		if data.to_users.length > 0
			for username in data.to_users
				user = TAGT.models.Users.findOneByUsername(username)
		...
		TAGT.models.Messages.findByRoomIdAndMessageIds(data.rid, data.messages, { sort: { ts: 1 } }).forEach (message) ->
			dateTime = moment(message.ts).locale(data.language).format('L LT')
			html += "<p style='margin-bottom: 5px'><b>#{message.u.username}</b> <span style='color: #aaa; font-size: 12px'>#{dateTime}</span><br />" + TAGT.Message.parse(message, data.language) + "</p>"
