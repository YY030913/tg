/* globals FileSystemStore:true, FileUpload, UploadFS, TAGTFile */
...
let createFileSystemStore = _.debounce(function() {
	...
	FileSystemStore = new UploadFS.store.Local({
		...
		name: storeName,
		path: TAGT.settings.get('FileUpload_FileSystemPath'), //'/tmp/uploads/photos',
		filter: new UploadFS.Filter({
			onCheck: FileUpload.validateFileUpload
		}),
		transformWrite: function(readStream, writeStream, fileId, file) {
			if (TAGTFile.enabled === false || !/^image\/((x-windows-)?bmp|p?jpeg|png)$/.test(file.type)) {
			...
			const identify = function(err, data) {
				...
				if ([null, undefined, '', 'Unknown', 'Undefined'].indexOf(data.Orientation) === -1) {
					return TAGTFile.gm(stream).autoOrient().stream().pipe(writeStream);
				} else {
					return stream.pipe(writeStream);
				}
			};
...
TAGT.settings.get('FileUpload_FileSystemPath', createFileSystemStore);
