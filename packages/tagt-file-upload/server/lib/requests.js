...
TAGT.settings.get('FileUpload_ProtectFiles', function(key, value) {
...
WebApp.connectHandlers.use('/file-upload/', function(req, res, next) {
	...
	if (match[1]) {
		file = TAGT.models.Uploads.findOneById(match[1]);
		...
		if (file) {
			if (!Meteor.settings.public.sandstorm && protectedFiles) {
				...
				if (!(uid && token && TAGT.models.Users.findOneByIdAndLoginToken(uid, token))) {
					res.writeHead(403);
					res.end();
					return false;
				}
