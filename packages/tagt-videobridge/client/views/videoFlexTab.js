...
Template.videoFlexTab.onCreated(function() {
	...
	let configOverwrite = {
		desktopSharingChromeExtId: TAGT.settings.get('Jitsi_Chrome_Extension')
	};
	...
	this.autorun(() => {
		if (TAGT.settings.get('Jitsi_Enabled')) {
			if (TAGT.TabBar.getTemplate() === 'videoFlexTab') {
				if (TAGT.TabBar.isFlexOpen()) {
					let roomId = Session.get('openedRoom');

					let domain = TAGT.settings.get('Jitsi_Domain');
					let jitsiRoom = 'TAGT' + CryptoJS.MD5(TAGT.settings.get('uniqueID') + roomId).toString();
					let noSsl = TAGT.settings.get('Jitsi_SSL') ? false : true;
					...
					if (jitsiRoomActive !== null && jitsiRoomActive !== jitsiRoom) {
						...
						TAGT.TabBar.updateButton('video', { class: 'red' });
