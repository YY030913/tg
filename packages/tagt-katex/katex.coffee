...
class Katex
	...
	# Takes a tagt message and renders latex in its content
	...
	katex_enabled: ->
		return TAGT.settings.get('Katex_Enabled')

	dollar_syntax_enabled: ->
		return TAGT.settings.get('Katex_Dollar_Syntax')

	parenthesis_syntax_enabled: ->
		return TAGT.settings.get('Katex_Parenthesis_Syntax')


TAGT.katex = new Katex

cb = TAGT.katex.render_message.bind(TAGT.katex)
TAGT.callbacks.add 'renderMessage', cb, TAGT.callbacks.priority.HIGH - 1

if Meteor.isClient
	Blaze.registerHelper 'TAGTKatex', (text) ->
		return TAGT.katex.render_message text
