...
TAGT.theme = new class
	...
	constructor: ->
		...
		TAGT.settings.add 'css', ''
		TAGT.settings.addGroup 'Layout'

		TAGT.settings.onload 'css', Meteor.bindEnvironment (key, value, initialLoad) =>
			if not initialLoad
				Meteor.startup ->
		...
		Meteor.startup =>
			TAGT.settings.onAfterInitialLoad =>

				TAGT.settings.get '*', Meteor.bindEnvironment (key, value, initialLoad) =>
	...
	compile: ->
		...
		less.render content, options, (err, data) ->
			...
			TAGT.settings.updateById 'css', data.css
	...
	addVariable: (type, name, value, persist=true) ->
		...
		if persist is true
			...
			TAGT.settings.add "theme-#{type}-#{name}", value, config
	...
	getCss: ->
		return TAGT.settings.get('css') or ''
