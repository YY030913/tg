...
Meteor.startup ->
	Tracker.autorun ->
		...
		messageUsers = TAGT.models.Messages.find({rid: Session.get('openedRoom'), 'u.username': {$ne: Meteor.user().username}}, {fields: {'u.username': 1, ts: 1}, sort: {ts: -1}}).fetch()
...
Template.messagePopupConfig.helpers
		...
		config =
			...
			getFilter: (collection, filter, cb) ->
				...
				if items.length < 5 and filter?.trim() isnt ''
					messageUsers = _.pluck(items, 'username')
					Tracker.nonreactive ->
						roomUsernames = TAGT.models.Rooms.findOne(Session.get('openedRoom')).usernames
	...
	popupChannelConfig: ->
		...
		config =
			title: t('Channels')
			collection: TAGT.models.Subscriptions
			trigger: '#'
			suffix: ' '
	...
	popupSlashCommandsConfig: ->
		...
		config =
			title: t('Commands')
			collection: TAGT.slashCommands.commands
			trigger: '/'
			suffix: ' '
		...
		if TAGT.emoji?
			...
			config =
				...
				getFilter: (collection, filter, cb) ->
					...
					if TAGT.emoji.packages.emojione?.asciiList[key] or filter.length < 2
						return []
