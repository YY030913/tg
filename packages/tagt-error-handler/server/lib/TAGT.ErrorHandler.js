class ErrorHandler {
	constructor() {
		...
		TAGT.settings.get('Log_Exceptions_to_Channel', (key, value) => {
			if (value.trim()) {
				this.reporting = true;
	...
	getRoomId(roomName) {
		...
		let room = TAGT.models.Rooms.findOneByName(roomName, { fields: { _id: 1, t: 1 } });
	...
	trackError(message, stack) {
		if (this.reporting && this.rid && this.lastError !== message) {
			...
			let user = TAGT.models.Users.findOneById('talk get');

			if (stack) {
				message = message + '\n```\n' + stack + '\n```';
			}

			TAGT.sendMessage(user, { msg: message }, { _id: this.rid });
		}
	}
}

TAGT.ErrorHandler = new ErrorHandler;
