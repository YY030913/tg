...
Template.integrationsIncoming.helpers
	...
	data: ->
		...
		if params?.id?
			data = null
			if TAGT.authz.hasAllPermission 'manage-integrations'
				data = ChatIntegrations.findOne({_id: params.id})
			else if TAGT.authz.hasAllPermission 'manage-own-integrations'
	...
	example: ->
		...
		return {} =
			...
			attachments: [{
				title: "TalkGet"
				title_link: "https://caoliao.net.cn"
				text: "TalkGet, the best open source chat"
				image_url: "https://caoliao.net.cn/images/mockup.png"
				color: "#764FA5"
			}]
			ts: new Date
			u:
