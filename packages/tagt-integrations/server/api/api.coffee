...
getIntegrationScript = (integration) ->
	...
	try
		...
		throw TAGT.API.v1.failure 'error-evaluating-script'

	if not sandbox.Script?
		logger.incoming.error '[Class "Script" not in Trigger', integration.name, ']'
		throw TAGT.API.v1.failure 'class-script-not-found'
...
Api = new Restivus
	...
	auth:
		user: ->
			...
			@integration = TAGT.models.Integrations.findOne
			...
			user = TAGT.models.Users.findOne
...
removeIntegration = (options, user) ->
	...
	integrationToRemove = TAGT.models.Integrations.findOne urls: options.target_url
	Meteor.runAsUser user._id, =>
...
executeIntegrationRest = ->
	...
	if @integration.scriptEnabled is true and @integration.scriptCompiled? and @integration.scriptCompiled.trim() isnt ''
		...
		try
			...
			if result?.error?
				return TAGT.API.v1.failure result.error
			...
			logger.incoming.error "[Stack:]"
			logger.incoming.error e.stack.replace(/^/gm, '  ')
			return TAGT.API.v1.failure 'error-running-script'

	if not @bodyParams?
		return TAGT.API.v1.failure 'body-empty'
...
TAGT.API.v1.addRoute 'integrations.create', authRequired: true,
	post: ->
		return createIntegration @bodyParams, @user


TAGT.API.v1.addRoute 'integrations.remove', authRequired: true,
	post: ->
		return removeIntegration @bodyParams, @user
