this.processWebhookMessage = function(messageObj, user, defaultValues) {
	...
	for (channel of channels) {
		...
		switch (channelType) {
			case '#':
				room = TAGT.models.Rooms.findOne({
					$or: [
						{
							_id: channel
						}, {
				...
				break;
			case '@':
				roomUser = TAGT.models.Users.findOne({
					$or: [
						{
							_id: channel
						}, {
				...
				if (!room) {
					Meteor.runAsUser(user._id, function() {
						...
						room = TAGT.models.Rooms.findOne(rid);
					});
				}
				break;
			default:
		...
		var messageReturn = TAGT.sendMessage(user, message, room);
