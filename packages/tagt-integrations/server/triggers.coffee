...
TAGT.models.Integrations.find({type: 'webhook-outgoing'}).observe
	added: (record) ->
		addIntegration(record)
...
ExecuteTriggerUrl = (url, trigger, message, room, tries=0) ->
	...
	sendMessage = (message) ->
		user = TAGT.models.Users.findOneByUsername(trigger.username)
	...
	opts =
		...
		auth: undefined
		npmRequestOptions:
			rejectUnauthorized: !TAGT.settings.get 'Allow_Invalid_SelfSigned_Certs'
	...
	HTTP.call opts.method, opts.url, opts, (error, result) ->
		...
		if not result? or result.statusCode not in [200, 201, 202]
			...
			if result.statusCode is 410
				TAGT.models.Integrations.remove _id: trigger._id
				return
...
TAGT.callbacks.add 'afterSaveMessage', ExecuteTriggers, TAGT.callbacks.priority.LOW, 'ExecuteTriggers'
