Meteor.methods
	addOutgoingIntegration: (integration) ->
		...
		if (not TAGT.authz.hasPermission @userId, 'manage-integrations') and
		not (TAGT.authz.hasPermission @userId, 'manage-own-integrations') and
		not (TAGT.authz.hasPermission @userId, 'manage-integrations', 'bot') and
		not (TAGT.authz.hasPermission @userId, 'manage-own-integrations', 'bot')
		...
		user = TAGT.models.Users.findOne({username: integration.username})
		...
		integration._createdBy = TAGT.models.Users.findOne @userId, {fields: {username: 1}}

		integration._id = TAGT.models.Integrations.insert integration
