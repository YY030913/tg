...
class IrcClient
	...
	onReceiveMessage: (source, target, content) ->
		...
		if target[0] == '#'
			room = TAGT.models.Rooms.findOneByName target.substring(1)
		else
		...
		TAGT.sendMessage source, message, room
	...
	onEndMemberList: (roomName) ->
		...
		room = TAGT.models.Rooms.findOneByNameAndType roomName, 'c'
		unless room
			return
		...
		TAGT.models.Rooms.removeUsernamesById room._id, removeMembers
		TAGT.models.Rooms.addUsernamesById room._id, appendMembers
	...
	initRoomList: ->
		roomsCursor = TAGT.models.Rooms.findByTypeContainigUsername 'c', @user.username,
			fields:
				name: 1
				t: 1
	...
	createDirectRoomWhenNotExist: (source, target) ->
		...
		TAGT.models.Subscriptions.upsert
			rid: rid
...
class IrcSender
	constructor: (message) ->
		...
		room = TAGT.models.Rooms.findOneById message.rid, { fields: { name: 1, usernames: 1, t: 1 } }
...
TAGT.callbacks.add 'beforeValidateLogin', IrcLoginer, TAGT.callbacks.priority.LOW
TAGT.callbacks.add 'beforeSaveMessage', IrcSender, TAGT.callbacks.priority.LOW
TAGT.callbacks.add 'beforeJoinRoom', IrcRoomJoiner, TAGT.callbacks.priority.LOW
TAGT.callbacks.add 'beforeCreateChannel', IrcRoomJoiner, TAGT.callbacks.priority.LOW
TAGT.callbacks.add 'beforeLeaveRoom', IrcRoomLeaver, TAGT.callbacks.priority.LOW
TAGT.callbacks.add 'afterLogoutCleanUp', IrcLogoutCleanUper, TAGT.callbacks.priority.LOW
