...
Template.adminEmoji.onCreated(function() {
	...
	this.customemoji = function() {
		...
		return TAGT.models.EmojiCustom.find(query, { limit: limit, sort: { name: 1 }}).fetch();
	};
});
...
Template.adminEmoji.events({
	...
	['click .emoji-info'](e) {
		e.preventDefault();
		TAGT.TabBar.setTemplate('adminEmojiInfo');
		TAGT.TabBar.setData(TAGT.models.EmojiCustom.findOne({_id: this._id}));
		TAGT.TabBar.openFlex();
		TAGT.TabBar.showGroup('adminEmoji-selected');
	},
