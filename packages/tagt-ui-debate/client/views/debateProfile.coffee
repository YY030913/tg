Template.debateProfile.helpers
	allowDeleteOwnAccount: ->
		return TAGT.settings.get('Accounts_AllowDeleteOwnAccount')
	...
	allowUsernameChange: ->
		return TAGT.settings.get("Accounts_AllowUsernameChange") and TAGT.settings.get("LDAP_Enable") isnt true

	allowEmailChange: ->
		return TAGT.settings.get("Accounts_AllowEmailChange")

	usernameChangeDisabled: ->
		return t('Username_Change_Disabled')

	allowPasswordChange: ->
		return TAGT.settings.get("Accounts_AllowPasswordChange")

	passwordChangeDisabled: ->
		return t('Password_Change_Disabled')

Template.debateProfile.onCreated ->
	...
	@changePassword = (newPassword, callback) ->
		...
		if not newPassword
			...
			if !TAGT.settings.get("Accounts_AllowPasswordChange")
				toastr.remove();
	...
	@save = (currentPassword) ->
		...
		if _.trim($('#username').val()) isnt Meteor.user().username
			if !TAGT.settings.get("Accounts_AllowUsernameChange")
				toastr.remove();
		...
		if _.trim($('#email').val()) isnt Meteor.user().emails?[0]?.address
			if !TAGT.settings.get("Accounts_AllowEmailChange")
				toastr.remove();
...
Template.debateProfile.onRendered ->
	Tracker.afterFlush ->
		# this should throw an error-template
		FlowRouter.go("home") if !TAGT.settings.get("Accounts_AllowUserProfileChange")
		SideNav.setFlex "debateFlex"
		SideNav.openFlex()

Template.debateProfile.events
	...
	'click .delete-debate button': (e) ->
		e.preventDefault();
		if s.trim Meteor.user()?.services?.password?.bcrypt
			swal
				title: t("Are_you_sure_you_want_to_delete_your_debate"),
			...
			, (typedPassword) =>
				if typedPassword
					toastr.remove();
					toastr.warning(t("Please_wait_while_your_debate_is_being_deleted"));
