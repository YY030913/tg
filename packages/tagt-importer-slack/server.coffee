Importer.Slack = class Importer.Slack extends Importer.Base
	...
	prepare: (dataURI, sentContentType, fileName) =>
		...
		{image, contentType} = TAGTFile.dataURIParse dataURI
	...
	startImport: (importSelection) =>
		...
		Meteor.defer =>
			...
			for user in @users.users when user.do_import
				do (user) =>
					Meteor.runAsUser startedByUserId, () =>
						existantUser = TAGT.models.Users.findOneByEmailAddress user.profile.email
						if not existantUser
							existantUser = TAGT.models.Users.findOneByUsername user.name

						if existantUser
							user.rocketId = existantUser._id
							TAGT.models.Users.update { _id: user.rocketId }, { $addToSet: { importIds: user.id } }
							@userTags.push
								slack: "<@#{user.id}>"
							...
							Meteor.runAsUser userId, () =>
								...
								# Slack's is -18000 which translates to TalkGet's after dividing by 3600
								if user.tz_offset
									Meteor.call 'userSetUtcOffset', user.tz_offset / 3600

							TAGT.models.Users.update { _id: userId }, { $addToSet: { importIds: user.id } }

							if user.profile.real_name
								TAGT.models.Users.setName userId, user.profile.real_name
							#Deleted users are 'inactive' users in TalkGet
							if user.deleted
			...
			for channel in @channels.channels when channel.do_import
				do (channel) =>
					Meteor.runAsUser startedByUserId, () =>
						existantRoom = TAGT.models.Rooms.findOneByName channel.name
						if existantRoom or channel.is_general
							...
							TAGT.models.Rooms.update { _id: channel.rocketId }, { $addToSet: { importIds: channel.id } }
						else
							users = []
							...
							TAGT.models.Rooms.update { _id: channel.rocketId }, { $set: roomUpdate, $addToSet: { importIds: channel.id } }
				...
				do (channel, messagesObj) =>
					Meteor.runAsUser startedByUserId, () =>
						...
						if slackChannel?.do_import
							room = TAGT.models.Rooms.findOneById slackChannel.rocketId, { fields: { usernames: 1, t: 1, name: 1 } }
							for date, msgs of messagesObj
										...
										if message.subtype?
											if message.subtype is 'channel_join'
												if @getRocketUser(message.user)?
													TAGT.models.Messages.createUserJoinWithRoomIdAndUser room._id, @getRocketUser(message.user), msgDataDefaults
											else if message.subtype is 'channel_leave'
												if @getRocketUser(message.user)?
													TAGT.models.Messages.createUserLeaveWithRoomIdAndUser room._id, @getRocketUser(message.user), msgDataDefaults
											else if message.subtype is 'me_message'
												msgObj =
													msg: "_#{@convertSlackMessageToTAGT(message.text)}_"
												_.extend msgObj, msgDataDefaults
												TAGT.sendMessage @getRocketUser(message.user), msgObj, room, true
											else if message.subtype is 'bot_message'
												botUser = TAGT.models.Users.findOneById 'talk get', { fields: { username: 1 }}
												botUsername = if @bots[message.bot_id] then @bots[message.bot_id]?.name else message.username
												msgObj =
													msg: @convertSlackMessageToTAGT(message.text)
													rid: room._id
													bot: true
												...
												TAGT.sendMessage botUser, msgObj, room, true
											else if message.subtype is 'channel_purpose'
												TAGT.models.Messages.createRoomSettingsChangedWithTypeRoomIdMessageAndUser 'room_changed_topic', room._id, message.purpose, @getRocketUser(message.user), msgDataDefaults
											else if message.subtype is 'channel_topic'
												TAGT.models.Messages.createRoomSettingsChangedWithTypeRoomIdMessageAndUser 'room_changed_topic', room._id, message.topic, @getRocketUser(message.user), msgDataDefaults
											else if message.subtype is 'pinned_item'
												if message.attachments
													msgObj =
														attachments: [
															"text" : @convertSlackMessageToTAGT message.attachments[0].text
															"author_name" : message.attachments[0].author_subname
															"author_icon" : getAvatarUrlFromUsername(message.attachments[0].author_subname)
														]
													_.extend msgObj, msgDataDefaults
													TAGT.models.Messages.createWithTypeRoomIdMessageAndUser 'message_pinned', room._id, '', @getRocketUser(message.user), msgObj
												else
													#TODO: make this better
													@logger.debug('Pinned item with no attachment, needs work.');
													#TAGT.models.Messages.createWithTypeRoomIdMessageAndUser 'message_pinned', room._id, '', @getRocketUser(message.user), msgDataDefaults
	...
	getRocketUser: (slackId) =>
		for user in @users.users when user.id is slackId
			return TAGT.models.Users.findOneById user.rocketId, { fields: { username: 1 }}
