...
@KonchatNotification =
	...
	notify: (notification) ->
		if window.Notification && Notification.permission == "granted"
			...
			TAGT.promises.run('onClientMessageReceived', message).then (message) ->
				...
				notificationDuration = (notification.duration - 0) or (Meteor.user()?.settings?.preferences?.desktopNotificationDuration - 0) or TAGT.settings.get('Desktop_Notifications_Duration')
