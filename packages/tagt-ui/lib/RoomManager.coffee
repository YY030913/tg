	...
	close = (typeName) ->
		if openedRooms[typeName]
			...
			if openedRooms[typeName].rid?
				...
				TAGT.Notifications.unRoom openedRooms[typeName].rid, 'deleteMessage', onDeleteMessageStream
			...
			do (typeName, record) ->
				...
				if ready is true
					...
					room = Tracker.nonreactive =>
						return TAGT.roomTypes.findRoom(type, name, user)
					...
					if not room?
						...
						if openedRooms[typeName].streamActive isnt true
							...
							msgStream.on openedRooms[typeName].rid, (msg) ->
								...
								TAGT.promises.run('onClientMessageReceived', msg).then (msg) ->
									...
									if RoomHistoryManager.hasMoreNext(openedRooms[typeName].rid) is false
										...
										TAGT.callbacks.run 'streamMessage', msg

										window.fireGlobalEvent('new-message', msg);

							TAGT.Notifications.onRoom openedRooms[typeName].rid, 'deleteMessage', onDeleteMessageStream
