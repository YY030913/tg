@MsgTyping = do ->
	...
	addStream = (room) ->
		if _.isEmpty usersTyping[room]?.users
			...
			TAGT.Notifications.onRoom room, 'typing', (username, typing) ->
	...
	start = (room) ->
		...
		renew = false
		selfTyping.set true
		TAGT.Notifications.notifyRoom room, 'typing', Meteor.user()?.username, true
	...
	stop = (room) ->
		...
		if timeouts?[room]?
			clearTimeout(timeouts[room])
			timeouts[room] = null
		TAGT.Notifications.notifyRoom room, 'typing', Meteor.user()?.username, false
