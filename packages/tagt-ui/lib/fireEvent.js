window.fireGlobalEvent = (eventName, params) => {
	...
	if (TAGT.settings.get('Iframe_Integration_send_enable') === true) {
		parent.postMessage({
			eventName: eventName,
			data: params
		}, TAGT.settings.get('Iframe_Integration_send_target_origin'));
	}
};

window.addEventListener('message', (e) => {
	if (TAGT.settings.get('Iframe_Integration_receive_enable') !== true) {
		return;
	}
	...
	let origins = TAGT.settings.get('Iframe_Integration_receive_origin');
