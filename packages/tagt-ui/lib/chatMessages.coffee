class @ChatMessages
	init: (node) ->
		this.editing = {}
		this.records  = {}
		this.messageMaxSize = TAGT.settings.get('Message_MaxAllowedSize')
	...
	resize: ->
		dif = (if TAGT.Layout.isEmbedded() then 0 else 60) + $(".messages-container").find("footer").outerHeight()
		$(".messages-box").css
	...
	send: (rid, input, done = ->) ->
		if _.trim(input.value) isnt ''
			...
			TAGT.promises.run('onClientBeforeSendMessage', msgObject).then (msgObject) =>
				...
				if msg[0] is '/'
					...
					if match?
						if TAGT.slashCommands.commands[match[1]]
							commandOptions = TAGT.slashCommands.commands[match[1]]
							command = match[1]
							param = match[2]
						...
						if !TAGT.settings.get('Message_AllowUnrecognizedSlashCommand')
							invalidCommandMsg =
								_id: Random.id()
								rid: rid
								ts: new Date
	...
	confirmDeleteMsg: (message, done = ->) ->
		return if TAGT.MessageTypes.isSystemMessage(message)
		swal {
