...
favoritesEnabled = ->
	return TAGT.settings.get 'Favorite_Rooms'

userCanDrop = (_id) ->
	return !TAGT.roomTypes.readOnly _id, Meteor.user()

Template.room.helpers
	embeddedVersion: ->
		return TAGT.Layout.isEmbedded()
	...
	roomName: ->
		...
		return TAGT.roomTypes.getRoomName roomData?.t, roomData
	...
	roomIcon: ->
		...
		return TAGT.roomTypes.getIcon roomData?.t
	...
	maxMessageLength: ->
		return TAGT.settings.get('Message_MaxAllowedSize')
	...
	flexTemplate: ->
		return TAGT.TabBar.getTemplate()
	...
	adminClass: ->
		return 'admin' if TAGT.authz.hasRole(Meteor.userId(), 'admin')
	...
	canPreview: ->
		...
		if TAGT.authz.hasAllPermission('preview-c-room')
			return true

		return TAGT.models.Subscriptions.findOne({rid: this._id})?

	buttons: ->
		visibleGroup = TAGT.TabBar.getVisibleGroup()
		showbuttons = []
		TAGT.OptionTabBar.getButtons().forEach (button) ->
			if button.groups.indexOf(visibleGroup) isnt -1
				showbuttons.push button
		return showbuttons

	route: ->
		return TAGT.OptionTabBar.getRouteLink @id, {rid: Template.instance().data._id} 
...
Template.room.events
	...
	"click .messages-container": (e) ->
		if TAGT.TabBar.isFlexOpen() and Meteor.user()?.settings?.preferences?.hideFlexTab then TAGT.TabBar.closeFlex()
	...
	'click .message-cog': (e) ->
		...
		if dropDown.length is 0
			actions = TAGT.MessageAction.getButtons message, 'message'
	...
	'click .message-dropdown .message-action': (e, t) ->
		...
		button = TAGT.MessageAction.getButtonById el.data('id')
		if button?.action?
	...
	"click #join-video": (e, t) ->
		if (ChatRoom.findOne(t.data._id).did? && TAGT.models.Debate.findOne(ChatRoom.findOne(t.data._id).did).webrtcJoined.length == 0) && Session.get('remoteCallData')?
		...
		Tracker.autorun ->
			...
			if debateSubscription.ready()
				if ChatRoom.findOne(template.data._id)?.did?
					if TAGT.models.Debate.findOne(ChatRoom.findOne(template.data._id).did).webrtcJoined.length > 0 && Session.get('remoteCallData')
